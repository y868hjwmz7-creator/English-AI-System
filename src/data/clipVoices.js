 ...
 * | `stability`        | 読み方をどれだけ揃えるか   | **0.5(Natural)。** v3 で**元の録音にいちばん近い**読み方(第5.192節) |
 ...
 */
/* **`stability` は 0.35 → 0.5**(第5.192節・2026-09 実機・利用者の指定)。

     > スコットランドの訛りで教材を作ったのに、全然訛りがなくなっている。
     > Elevenlabs のスピーカーたちの設定を
     > **もともとの訛りを最大限生かす設定**に直してください。

   ── **なぜ 0.5 なのか** ──────────────────────────────────

   **v3 が受け取る `stability` は、とびとびの3つだけ**である(0 / 0.5 / 1)。
   ElevenLabs はそれぞれをこう呼んでいる。

   | 値 | 呼び名 | 何が起きるか |
   |---|---|---|
   | 0   | Creative | いちばん感情が出る。**そのぶん元の録音から離れる** |
   | 0.5 | Natural  | **元の録音にいちばん近い読み方** |
   | 1   | Robust   | いちばん揃う。感情の指定が効きにくい |

   **訛りは「元の録音の特徴」そのもの**なので、
   **最大限に活かす = 元の録音にいちばん近い = 0.5(Natural)** である。

   ── **0.35 は、そもそも3つのどれでもなかった** ──────────────

   前の値 0.35 は利用者の別の指定(「感情が豊かな方が良いです」)から
   置いたものだが、**v3 の3つのどれにも当たらない。**
   窓口は先回りして丸めない(`snapStability` の節)ので、
   **断られたときだけ 0.5 に寄る。** 断られなければ低いほうに扱われ、
   **Creative 寄り = 元の録音から離れる** —— 訛りが薄くなる側である。

   **つまり「感情を出す」と「訛りを残す」は、同じつまみの両端である。**
   今回は利用者が**訛りを最大限**と指定したので、そちらを取る。
   感情のほうが欲しくなったら、ここを 0 に戻せばよい
   (**その代わり、訛りはまた薄くなる**)。

   ── **変えたら、音声は作り直しになる** ────────────────────

   置き場所は(版, 段, 声の id, 英文の指紋)で決まる。
   **設定は入っていないので、自動では作り直らない。**
