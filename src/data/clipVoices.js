...
/**
 * ElevenLabs のモデル名。**名簿の `model` にはこの値を書く。**
 *
 * **短い書き方(`'v2'` など)を作らない。** 名簿と窓口の2か所で
 * 同じ対応表を持つことになり、必ずずれる(`CLIP_REV` で踏んだ話と同じ)。
 * ここに書いた文字列が、そのまま ElevenLabs へ渡る。
 */
export const V3 = 'eleven_v3'
export const V2 = 'eleven_multilingual_v2'
...
export const CLIP_VOICES = [
  ...
  /* **雑音の出どころは、この声そのものだった**(2026-09 実機・利用者の判断)。
     ...
     **理由は「効かなかった」ではなく、「v3 には無い欄だった」である。**
     ElevenLabs の説明にこうある(2026-09 に確かめた)。

       Similarity is not available for the Eleven v3 model.
       Speaker Boost is not available for the Eleven v3 model.

     **つまり v3 では、この2つはどちらも読まれずに捨てられる。**
     `speed` も同じで、だから声の速さを `playbackRate` で直している
     (`voiceRateOf` の節)。利用者が使う声は**すべて v3** なので、
     **この実験は、何度作り直しても永久に何も変わらない。**

     **前の 0.1 の実験も、これで説明が付いた。** あのときは窓口が
     `eleven_multilingual_v2` を頼んでいたと思っていたが、
     どちらのモデルだったにせよ、**いま同じことをしても変わらない。**

     **だから値を消した。** 効かない欄を書いたまま残すと、
     **次に見た人が「もう試した」と読み、また同じ課金をする。**
     1人だけ値を変える道(その行の `settings`)はそのまま使えるが、
     **動かして意味があるのは `stability` だけ**である。 */
  ...
  /* **この声だけ v2 で聴いて選ばれている**(2026-09 利用者の指定
 ...
 *
 * 【なぜ声ごとに持つのか】
 *   利用者は ElevenLabs の画面で**実際に聴いてから**声を選んでいる。
 *   v2 の画面で聴いた声を v3 で鳴らせば、**聴いた音とは別物**になる。
 *   モデルは声に付いているのではなく、**こちらが呼ぶときに決めている**
 *   ので、**聴いたほうを名簿に書き残すしかない。**
 *
 * 【書かなければ v3】
 *   利用者が使う声は原則すべて v3 である(CLAUDE.md)。
 *   **書いていない声は、これまでどおり v3。** 1本も作り直しにならない。
 *
 * 【モデルで、できることが逆になる】
 *   | | v2 | v3 |
 *   |---|---|---|
 *   | 感情のコマンド `[excited]` | **使えない**(文字がそのまま読まれる) | 使える |
 *   | `similarity_boost` / `use_speaker_boost` | **効く** | 効かない |
 *   | `<break time="1.5s" />` | 使える | 使えない |
 *
 *   だから**感情のコマンドを入れるときは、v2 の声には入れない。**
 *   逆に**雑音を減らす調整(似せ具合を下げる)ができるのは v2 だけ**である。
 *
 * 【置き場所には入っていない】
 *   `<版>/<段>/<声の id>/<英文の指紋>.mp3` にモデルは入らない。
 *   **新しく足す声には既存の音声が無い**ので、これで何も壊れない。
 *   すでにある声のモデルを**あとから変えたときだけ**、
 *   その声の音声を「読み上げ音声を作り直す」で作り直す必要がある。
 */
export const voiceModelOf = (id) => findVoice(id)?.model ?? V3
 ...
 * | `stability`        | 読み方をどれだけ揃えるか   | **0.35。** 実機で**訛りがいちばん濃かった値**(第5.258節)。数字の意味では選んでいない |
 ...
 */
/* **`stability` は 0.5 → 0.35 に戻した**(第5.258節・2026-09-25 実機・利用者の聴き比べ)。

     > 訛りの設定を訛りを活かすにした時の訛り方が以前よりも弱いです。
     > 添付の写真の教材の時はスコットランドの男性、Mark の訛りが非常に
     > 良かったのに、今は普通のイギリス人くらいきれいな発音になって
     > しまっています。比較して「訛りを活かす」については
     > **全てのスピーカーに対して以前の設定に直してください。**

   ── **「以前」とは、いつの音か** ──────────────────────────

   利用者が挙げた教材は **2026-09-11**(`Wind Turbine Overheating Fault
   Response`)。**履歴を当たった。** あの日の値は **0.35** で、
   0.5 にしたのは 2026-09-18(第5.192節)である。

   **2026-09-11 と今のあいだで、訛りに関わる違いは `stability` だけ。**
   名簿の Mark の行(`sc-5`)も、モデルの既定(v3)も、窓口の梯子も、
   1文字も変わっていない。**だから 0.35 に戻せば、あの音に戻る。**

   ── **第5.192節の読みが外れていた** ─────────────────────

   あのとき 0.5 を選んだのは、ElevenLabs の呼び名から**推しただけ**である。

   | 値 | 呼び名 | あのとき、こう読んだ |
   |---|---|---|
   | 0   | Creative | 感情が出る。**元の録音から離れる** |
   | 0.5 | Natural  | **元の録音にいちばん近い**(= 訛りが残る、と考えた) |
   | 1   | Robust   | 揃う。感情の指定が効きにくい |

   **「元の録音に近い」= 「訛りが濃い」ではなかった。**
   実際に聴くと、**0.35 のほうが訛りが濃い。**
   第5.192節の節には「**聞いて確かめていない**」と書いてあった ——
   こちらには音が聞こえないので、**数字の意味から選んだ値**だった。
   **耳のほうが正しい。**

   ── **0.35 が、窓口でどう扱われているかは分かっていない** ────────

   窓口(`supabase/functions/speak`)は上から順に3つ試す。

     ① 頼まれたまま(v3 ＋ `stability: 0.35`)
     ② v3 のときだけ … 3つの値(0 / 0.5 / 1)に寄せて
     ③ v2 ＋ 頼まれたまま

   **どの段で通ったのかは、こちらでは確かめられない**(この環境から
   ElevenLabs に届かない)。①なら v3 が 0.35 をそのまま受けており、
   ③なら v2 で鳴っている —— **v2 は `similarity_boost` と
   `use_speaker_boost` を読む**ので、そちらでも訛りは濃くなる。
   **分かっていないことを、分かったように書かない**(CLAUDE.md)。

   **確かなのは1つ。** 0.35 に戻せば、**2026-09-11 とまったく同じ梯子**を
   まったく同じ順で通る。**利用者が良いと言った音は、それで返る。**

   ── **窓口で先回りして丸めない**(ここを崩すと、また 0.5 に戻る)──

   ②は「断られたとき」だけの受け皿である。もし窓口が**送る前に**
   0.35 を 0.5 へ寄せる形に変わったら、ここを 0.35 にしても意味が無くなる。
   `npm run test:voice` が、梯子の1段目が「頼まれたまま」であることを見張る。

   ── **感情が欲しいときは、画面で選ぶ** ──────────────────

   「感情を出す」(`-emo`)は `stability: 0` で、**別の音声として作られる**
   (声の id の後ろに印が付き、置き場所が別になる)。
   **ここを動かして感情を出そうとしない。** それは全員に効いてしまう。

   ── **変えても、すでにある音声は作り直らない** ────────────────

   置き場所は(版, 段, 声の id, 英文の指紋)で決まる。**設定は入っていない。**
   これまでに作った音声は 0.5 のままなので、
   さがす画面の「**読み上げ音声を作り直す**」を押してもらう
   (**押したぶんだけ課金される。** 押す前に本数と金額が出る)。
   これから作る教材は、何もしなくても 0.35 で作られる。 */
/* **v3 が実際に読むのは、このうち `stability` だけである**(2026-09 に確認)。
   ElevenLabs の説明にこうある。

     Similarity is not available for the Eleven v3 model.
     Speaker Boost is not available for the Eleven v3 model.

   `style` は「v2+ and v3 models only」と書かれているので読まれる。
   **利用者が使う声はすべて v3** なので、`similarity_boost` と
   `use_speaker_boost` は**渡っても捨てられる。**

   **それでも4つとも残してある。**
     ・**渡す道が切れていないこと**を `npm run test:voice` が見張っている。
       欄を減らすと、その見張りが見るものを失う
     ・v2 に落ちたとき(v3 がプランで使えない・断られた)には効く
     ・**捨てられるだけで、害は無い**(窓口が 0〜1 に丸めて渡す)

   **害があるのは「効くと思って値をいじること」のほうである。**
   Ally で2度それをやり、2度とも課金だけがかかった(`sc-1` の節)。
   **音を変えたいなら `stability` を動かす。ほかの3つは動かさない。** */
export const ACCENT_KEEP = {
  similarity_boost: 1,
  /* **0.35。** 3つ(0 / 0.5 / 1)のどれでもないのは**わざと**である ——
     実機で訛りがいちばん濃かった値(第5.258節)。上の節を読んでから動かす */
  stability: 0.35,
  style: 0,
 ...
 *
 * 【なぜ「どちらか」なのか】
 *   どちらも `stability` という**同じつまみ1本**で決まる。
 *   片方を選べば、もう片方は諦めることになる(第5.192節)。
 *   **両立させる値は無い。** だから「都度えらぶ」形にした。
 *
 * | id | 画面に出す名前 | stability | どうなるか |
 * |---|---|---|---|
 * | `accent`(既定) | 訛りを活かす | **0.35** | **実機で訛りがいちばん濃かった値**(第5.258節) |
 * | `emotion` | 感情を出す | **0** Creative | 抑揚が大きい。**訛りは薄れる** |
 *
 * **1 (Robust) は出さない。** 揃うだけで、訛りも感情もどちらも
 * 前に出ない —— **えらぶ意味のある両端だけを見せる。**
 *
 * 【0.35 は、v3 の3つ(0 / 0.5 / 1)のどれでもない。**わざとである**】
 *   第5.192節では「呼び名から推して」 0.5(Natural)にしたが、
 *   **利用者が聴き比べて、0.35 のほうが訛りが濃いと分かった**(第5.258節)。
 *   **数字の意味より、耳が先**である。くわしくは `ACCENT_KEEP` の節。
 *
 * 【画面に出す名前は「声の出し方」】
