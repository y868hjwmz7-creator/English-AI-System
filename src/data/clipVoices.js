...
export const CLIP_VOICES = [
  ...
  /* **雑音の出どころは、この声そのものだった**(2026-09 実機・利用者の判断)。
     ...
     **代わりに Hugh が入っている**(下)。スコットランド訛りを
     ...
     ── **2026-09、もう一度試している**(利用者の指定)──────────

     > Alley に対してのみ元の音声のキャラクターを残すを 0.1 にしてみて
     > ください。それで雑音が減るかのテストです。
     > 残すを Off の方が実験としては良いかもしれません

     **前の実験は、根拠になっていなかった。** あのとき窓口は
     `eleven_multilingual_v2` を頼んでいた(2026-09-04b で v3 にした)。
     **利用者が聴いて選んだ音とは別のモデルの出力**を聴いて
     「変わらなかった」と判断していたことになる。
     **v3 になって音が変わったことは、利用者が実機で確かめている**
     (「かなり感情がのり、不自然なポーズも減りました」)。
     だから**同じ実験をもう一度やる意味がある。**

     下の `settings` は、**この声にだけ**効く(`voiceSettingsOf`)。
     ほかの声は既定(`ACCENT_KEEP`)のままで、1ミリも変わらない。

     **`stability` は触らない**(0.35 のまま)。感情が出ているのは
     いまの値のおかげなので、**一度に2つ動かすと、どちらが効いたのか
     分からなくなる。** 動かすのは「もとの録音にどれだけ寄せるか」だけ。

     **「残す」を Off にするのは、2つある。** どちらも
     「もとの録音にどれだけ寄せるか」を上げる指定なので、
     **片方だけでは Off にならない。**

       `similarity_boost: 0`      … もとの録音に寄せない
       `use_speaker_boost: false` … その話者らしさを強めない

     もとの録音に息の音や部屋鳴りが入っているなら、これで減るはずである。
     **引き換えに訛りは薄くなる**(スコットランドらしさが落ちる)。
     **雑音が消えるかどうかを見るための実験であって、この値のまま
     使うつもりのものではない。** 消えなければ外す。 */
  { id: 'sc-1', accent: 'sc', gender: 'male', use: 'both', retired: true,
    label: 'Ally', elevenId: 'v2zbX16tJNtRIx8rSHDM',
    settings: { similarity_boost: 0, use_speaker_boost: false } },
