...
/**
 * **スピーチ(モノローグ)の場面**(2026-09 利用者の指定)。
 *
 *   > 自分でスピーチなどを考えてもらったものをそのままコピペして
 *   > 指定する音声で text to speech をして、オーバーラッピングや
 *   > シャドーイングのように練習できるモードが欲しいです。
 *   > …業界とシチュエーションなどを選べばそれに合わせた Speech を
 *   > 作ってくれるのも最高です。
 *   > 場面などはあなたが考えて実装して下さい。さまざまな場面があるはずです。
 *
 * **会話の場面(`DIALOGUE_SCENES`)は使えない。**
 * あちらは「相手がいて、やりとりが続く」場面である。
 * スピーチは**1人が最後まで話しきる**ので、
 * 「交渉」「トラブル対応」を選んでも、そういう原稿にはならない。
 * だから**別の一覧を持つ。既存の場面は1つも減らしていない。**
 *
 * **長さと調子がばらばらなものを、わざと混ぜてある。**
 * 30秒の乾杯のあいさつと、15分の学会発表では、
 * 使う言い回しも、間の取り方もまったく違う。
 * 練習したい場面が無いと、この機能そのものが使われない。
 */
export const SPEECH_SCENES = [
  { id: 'sp_intro',      label: '自己紹介(初対面の場)',
    hint: '名前・所属・何をしている人か。30秒から1分' },
  { id: 'sp_newjob',     label: '着任のあいさつ',
    hint: '新しい部署・拠点で。これから何をするかを短く' },
  { id: 'sp_standup',    label: '朝礼・週次でのひとこと',
    hint: '今週やること。1分。数字を1つだけ入れる' },
  { id: 'sp_progress',   label: '進捗報告のプレゼン',
    hint: '数字と、次にやること。良い話だけにしない' },
  { id: 'sp_proposal',   label: 'お客様への提案',
    hint: '相手の困りごとから入り、こちらの案へつなぐ' },
  { id: 'sp_product',    label: '製品・サービスの紹介',
    hint: '新しいところを1つに絞る。並べると何も残らない' },
  { id: 'sp_booth',      label: '展示会のブースでの説明',
    hint: '立ち止まった人へ、30秒で。専門用語を避ける' },
  { id: 'sp_pitch',      label: '投資家へのピッチ',
    hint: '3分。困りごと・解き方・数字・お願い、の順' },
  { id: 'sp_conference', label: '学会・カンファレンスでの発表',
    hint: '背景・やり方・結果・これから、の順に' },
  { id: 'sp_training',   label: '研修・勉強会で教える',
    hint: '手順を1つずつ。聞き手に問いかけながら進める' },
  { id: 'sp_townhall',   label: '全社集会でのあいさつ',
    hint: '方針を、専門用語を使わずに。人に向けて話す' },
  { id: 'sp_award',      label: '表彰・受賞のあいさつ',
    hint: '短く。自分の手柄にせず、周りへの感謝を先に' },
  { id: 'sp_toast',      label: '乾杯のあいさつ',
    hint: '30秒。明るく、短く。長い話にしない' },
  { id: 'sp_farewell',   label: '送別・退職のあいさつ',
    hint: '感謝と、次へ。湿っぽくしすぎない' },
  { id: 'sp_apology',    label: 'お詫びと説明',
    hint: '事実・原因・これから、の順。言い訳から入らない' },
  { id: 'sp_interview',  label: '面接での自己 PR',
    hint: '経験と、いまできることを結びつける' },
  { id: 'sp_talk',       label: '講演(考えを伝える)',
    hint: '主張は1つ。それを自分の体験で支える' },
  /* **プレゼンまわりを足した**(2026-09 利用者の指定
     「スピーチ作成のシチュエーションに『プレゼン』などを追加して下さい」)。

     **いちばん下に足してある。** 上の17件は1つも消していないし、
     並びも1つも動かしていない(「一度入れたものを勝手に減らさない・
     並べ替えない」・CLAUDE.md)。 */
  { id: 'sp_deck',       label: 'プレゼン(スライドを使って)',
    hint: '1枚に伝えることは1つ。いま何枚目の何の話かを、そのつど言う' },
  { id: 'sp_demo',       label: 'デモ・実演',
    hint: '次に何をするかを先に言ってから動かす。画面の中を言葉でも説明する' },
  { id: 'sp_kickoff',    label: '案件のキックオフ説明',
    hint: '目的・進め方・誰が何をするか。日付と担当をはっきり言う' },
  { id: 'sp_result',     label: '決算・数字の報告',
    hint: '数字を先に、理由をあとに。増えた・減った理由は1つに絞る' },
  { id: 'sp_webinar',    label: 'オンライン説明会・ウェビナー',
    hint: '相手の顔が見えない前提。区切りごとに「ここまでで質問は?」と挟む' },
  { id: 'sp_handover',   label: '引き継ぎの説明',
    hint: 'いまの状態・残っていること・気をつけること、の順に' },
  ...
  /* ── あいさつの場面(2026-09 利用者の指定)────────────────────
     ...
     **後ろへ足す。並べ替えない**(docs/notes/22 の決まり)——
     前へ割り込ませると、覚えた並びが全部ずれる。

     もとからあるのは「自己紹介(初対面の場)」「着任のあいさつ」の2つで、
     **どちらも自分の話**である。足りなかったのは
     **相手先へ出向くとき・場を開くとき・締めるとき**のあいさつで、
     どれも**話す長さも、丁寧さも、入り方も違う。** */
  { id: 'sp_greet',      label: 'あいさつ(短いひとこと)',
    hint: '30秒。名乗って、ひとこと。長い話にしない' },
  { id: 'sp_client1st',  label: 'お客様との初顔合わせ',
    hint: '案件が決まって最初の場。自己紹介と、これから一緒にやることを短く' },
  { id: 'sp_visit',      label: '訪問先でのあいさつ',
    hint: '相手のオフィスで。時間をもらった礼から入り、用件へつなぐ' },
  { id: 'sp_welcome',    label: '来客を迎えるあいさつ',
    hint: '迎える側として。今日の流れを先に言う' },
  { id: 'sp_online',     label: 'オンライン会議の冒頭',
    hint: '顔が見えない前提。名乗り、今日のゴールを1つ言う' },
  { id: 'sp_newyear',    label: '年始・期初のあいさつ',
    hint: '去年の礼と、今年やること。数字は1つだけ' },
  { id: 'sp_social',     label: '懇親会・会食でのあいさつ',
    hint: 'くだけた場。ひとこと感謝して、場を渡す' },
  { id: 'sp_thanks',     label: '受注・契約が決まったときのお礼',
    hint: '選んでもらった礼と、次にやること。売り込みに戻らない' },
  { id: 'sp_closing',    label: '締めのあいさつ',
    hint: '会や案件の終わりに。決まったことを1つだけ確かめて閉じる' },
]

/**
 * **趣味の集まりでのスピーチ**(仕事の場面をそのまま出さないため)。
 *
 * 「投資家へのピッチ」をゴルフ仲間の集まりに出しても、
 * その人が英語を使う場面にはならない(`COMMON_HOBBY_SCENES` と同じ考え方)。
 */
export const COMMON_HOBBY_SPEECH_SCENES = [
  { id: 'sph_intro',   label: '集まりでの自己紹介',
    hint: '名前と、なぜここに来たのか。30秒' },
  { id: 'sph_wedding', label: '結婚式のスピーチ',
    hint: '二人との思い出を1つ。短くまとめる' },
  { id: 'sph_toast',   label: '乾杯のあいさつ',
    hint: '30秒。明るく、短く' },
  { id: 'sph_club',    label: 'クラブ・サークルの総会',
    hint: '1年の報告と、来年のお願い' },
  { id: 'sph_award',   label: '大会・発表会でのあいさつ',
    hint: '結果と、支えてくれた人への感謝' },
  { id: 'sph_share',   label: '好きなものをすすめる話',
    hint: 'なぜ好きかを、具体的な場面で語る' },
  { id: 'sph_travel',  label: '旅の報告',
    hint: '出来事を時間の順に。写真を見せながら話す調子で' },
]
...
export const SCENES_BY_INDUSTRY = {
  ...
  ene_re: [
    ...
    /* ── 土木の場面(第5.188節・2026-09 利用者の指定)───────────────
         > 「業界」内のエネルギー関連すべてに「土木」の要素をうまく
         > 追加してください。

       **発電所は、電気の話の前に土木の話がある。** 用地・造成・基礎・
       進入路・排水がまとまらなければ、1枚のパネルも1本の鉄塔も立たない。
       **後ろへ足す。並べ替えない**(docs/notes/22 の決まり) */
    { id: 'ren_land', label: '用地と造成の相談', hint: '測量、境界、切土と盛土、残土の行き先' },
    { id: 'ren_civil', label: '土木工事の見積り', hint: '造成、進入路、基礎。内訳と、増える理由' },
  ],
  ene_solar: [
    ...
    /* ── 土木の場面(第5.188節・2026-09 利用者の指定)───────────────
       ...
       **発電所は、電気の話の前に土木の話がある。** 用地・造成・基礎・
       進入路・排水がまとまらなければ、1枚のパネルも1本の鉄塔も立たない。
       **後ろへ足す。並べ替えない**(docs/notes/22 の決まり) */
    { id: 'sol_grade', label: '造成と排水', hint: '切土と盛土、法面、調整池、濁水' },
    { id: 'sol_found', label: '架台の基礎', hint: '杭、地耐力、支持層、引き抜きの力' },
  ],
  ene_wind: [
    ...
    /* ── 土木の場面(第5.188節・2026-09 利用者の指定)───────────────
       ...
       **発電所は、電気の話の前に土木の話がある。** 用地・造成・基礎・
       進入路・排水がまとまらなければ、1枚のパネルも1本の鉄塔も立たない。
       **後ろへ足す。並べ替えない**(docs/notes/22 の決まり) */
    { id: 'wnd_found', label: '基礎とアンカー', hint: '鉄筋、打設、養生、ボルトの締め付け' },
    { id: 'wnd_road', label: '進入路と搬入', hint: '曲線半径、勾配、重量制限、仮設の橋' },
  ],
  ene_hydro: [
    ...
    /* ── 土木の場面(第5.188節・2026-09 利用者の指定)───────────────
       ...
       **発電所は、電気の話の前に土木の話がある。** 用地・造成・基礎・
       進入路・排水がまとまらなければ、1枚のパネルも1本の鉄塔も立たない。
       **後ろへ足す。並べ替えない**(docs/notes/22 の決まり) */
    { id: 'hyd_civil', label: '堤体と水路の工事', hint: 'コンクリート、打継ぎ、漏水、点検路' },
    { id: 'hyd_tunnel', label: '導水トンネル', hint: '掘削、支保、湧水、覆工' },
  ],
  ene_therm: [
    ...
    /* ── 土木の場面(第5.188節・2026-09 利用者の指定)───────────────
       ...
       **発電所は、電気の話の前に土木の話がある。** 用地・造成・基礎・
       進入路・排水がまとまらなければ、1枚のパネルも1本の鉄塔も立たない。
       **後ろへ足す。並べ替えない**(docs/notes/22 の決まり) */
    { id: 'thm_berth', label: '桟橋と護岸', hint: '荷揚げ、防食、波、係船' },
    { id: 'thm_found', label: '基礎と地盤', hint: '杭、沈下、耐震、煙突の基礎' },
  ],
  ene_nuke: [
    ...
    /* ── 土木の場面(第5.188節・2026-09 利用者の指定)───────────────
       ...
       **発電所は、電気の話の前に土木の話がある。** 用地・造成・基礎・
       進入路・排水がまとまらなければ、1枚のパネルも1本の鉄塔も立たない。
       **後ろへ足す。並べ替えない**(docs/notes/22 の決まり) */
    { id: 'nuc_quake', label: '耐震と地盤の話', hint: '地盤調査、基準地震動、免震、支持層' },
    { id: 'nuc_seawall', label: '防潮堤の工事', hint: '高さの根拠、施工、止水、継ぎ目' },
  ],
  ene_geo: [
    ...
    /* ── 土木の場面(第5.188節・2026-09 利用者の指定)───────────────
       ...
       **発電所は、電気の話の前に土木の話がある。** 用地・造成・基礎・
       進入路・排水がまとまらなければ、1枚のパネルも1本の鉄塔も立たない。
       **後ろへ足す。並べ替えない**(docs/notes/22 の決まり) */
    { id: 'geo_road', label: '山の道と法面', hint: '取り付け道路、切土、落石、冬の通行' },
    { id: 'geo_pad', label: 'ヤードと配管の据付', hint: '整地、基礎、支持、伸縮' },
  ],
  ene_grid: [
    ...
    /* ── 土木の場面(第5.188節・2026-09 利用者の指定)───────────────
       ...
       **発電所は、電気の話の前に土木の話がある。** 用地・造成・基礎・
       進入路・排水がまとまらなければ、1枚のパネルも1本の鉄塔も立たない。
       **後ろへ足す。並べ替えない**(docs/notes/22 の決まり) */
    { id: 'grid_tower', label: '鉄塔の基礎', hint: '搬入、索道、深礎、用地の取得' },
    { id: 'grid_duct', label: '洞道と管路', hint: '推進、開削、道路占用、他企業の埋設物' },
  ],
  ene_batt: [
    ...
    /* ── 土木の場面(第5.188節・2026-09 利用者の指定)───────────────
       ...
       **発電所は、電気の話の前に土木の話がある。** 用地・造成・基礎・
       進入路・排水がまとまらなければ、1枚のパネルも1本の鉄塔も立たない。
       **後ろへ足す。並べ替えない**(docs/notes/22 の決まり) */
    { id: 'bat_found', label: '基礎と地盤改良', hint: '支持力、不同沈下、アンカー' },
    { id: 'bat_bund', label: '防油堤と排水', hint: '離隔、勾配、集水、消火の水' },
  ],
  ene_h2: [
    ...
    /* ── 土木の場面(第5.188節・2026-09 利用者の指定)───────────────
       ...
       **発電所は、電気の話の前に土木の話がある。** 用地・造成・基礎・
       進入路・排水がまとまらなければ、1枚のパネルも1本の鉄塔も立たない。
       **後ろへ足す。並べ替えない**(docs/notes/22 の決まり) */
    { id: 'h2_found', label: 'タンクとラックの基礎', hint: '荷重、耐震、不同沈下' },
    { id: 'h2_pipe', label: '配管を埋める', hint: '掘削、土被り、標識、他の埋設物' },
  ],
  ene_oil: [
    ...
    /* ── 土木の場面(第5.188節・2026-09 利用者の指定)───────────────
       ...
       **発電所は、電気の話の前に土木の話がある。** 用地・造成・基礎・
       進入路・排水がまとまらなければ、1枚のパネルも1本の鉄塔も立たない。
       **後ろへ足す。並べ替えない**(docs/notes/22 の決まり) */
    { id: 'oil_berth', label: '桟橋・護岸の工事', hint: '着桟、防舷材、浚渫、老朽化' },
    { id: 'oil_tank', label: 'タンクの基礎', hint: 'リング基礎、不同沈下、防油堤' },
  ],
 ...
 */
/**
 * **業界ごとのスピーチの場面**(第5.188節・2026-09 利用者の指定)。
 *
 *   > 「業界」内のエネルギー関連すべてに「土木」の要素をうまく
 *   > 追加してください。スピーチに対しても同様です。
 *
 * これまでスピーチの場面は**共通の一覧だけ**だった
 * (`SPEECH_SCENES`)。会話(`SCENES_BY_INDUSTRY`)や
 * 話題(`GENRES_BY_INDUSTRY`)と**同じ形**の表をここに足して、
 * 業界に特化した場面も選べるようにしてある。
 *
 * **引き当ても、会話とまったく同じ道を通す**(`ownOf` → 特化 + 共通)。
 * だから「全般」を選べば、種類ぜんぶのものが並ぶ。
 *
 * **登録が無い業界は、これまでどおり共通の一覧だけ**である
 * (1つも増えない・1つも減らない)。
 *
 * ## エネルギーの土木は、11 種で同じものを使う
 *
 *   会話の場面は、風力の「ブレード」と水力の「水車」のように
 *   **現場で交わす言葉がまるごと違う**ので1つずつ持っている。
 *   けれども**スピーチのほうは違う。**「住民へ工事の説明をする」
 *   「安全大会であいさつする」は、太陽光でも送電でも**同じ場面**である。
 *   **同じものを11か所に書き写さない**(CLAUDE.md)。
 */
const ENERGY_CIVIL_SPEECH = [
  { id: 'spc_plan',    label: '工事計画の説明(住民向け)',
    hint: '何を、いつからいつまで。生活に何が起きるかを先に言う' },
  { id: 'spc_kickoff', label: '着工前の合同打ち合わせ',
    hint: '工程、搬入路、重機。関係する業者へ、順番に' },
  { id: 'spc_safety',  label: '安全大会でのあいさつ',
    hint: '危ないところを1つに絞る。数字と、具体的な動きで' },
  { id: 'spc_check',   label: '検査・立会いでの説明',
    hint: '出来形、写真、記録。聞かれる前にこちらから言う' },
  { id: 'spc_env',     label: '環境への影響の説明',
    hint: '濁水、騒音、振動。測った数字と、手の打ち方' },
  { id: 'spc_disaster', label: '災害のあとの状況説明',
    hint: '被害、通れない道、復旧の見込み。分からないことは分からないと言う' },
]

/**
 * 業界 → スピーチの場面。**エネルギーの 11 種は、同じ一覧を分け合う。**
 * 親(`energy`)には持たせない —— `ownOf` が種類ぜんぶを集めるので、
 * 「全般」を選んでも同じものが並ぶ(**2か所に書かない**)。
 */
export const SPEECH_BY_INDUSTRY = {
  ene_re: ENERGY_CIVIL_SPEECH,
  ene_solar: ENERGY_CIVIL_SPEECH,
  ene_wind: ENERGY_CIVIL_SPEECH,
  ene_hydro: ENERGY_CIVIL_SPEECH,
  ene_therm: ENERGY_CIVIL_SPEECH,
  ene_nuke: ENERGY_CIVIL_SPEECH,
  ene_geo: ENERGY_CIVIL_SPEECH,
  ene_grid: ENERGY_CIVIL_SPEECH,
  ene_batt: ENERGY_CIVIL_SPEECH,
  ene_h2: ENERGY_CIVIL_SPEECH,
  ene_oil: ENERGY_CIVIL_SPEECH,
}
  ...
  ene_re: [
    ...
    /* ── 土木の話題(第5.188節・2026-09 利用者の指定)───────────────
       ...
       **発電所は、電気の話の前に土木の話がある。** 用地・造成・基礎・
       進入路・排水がまとまらなければ、1枚のパネルも1本の鉄塔も立たない。
       **後ろへ足す。並べ替えない**(docs/notes/22 の決まり) */
    { id: 'reng_civil', label: '造成をどこまでやるか', hint: '土を動かす量と、費用と、環境' },
  ],
  ene_solar: [
    ...
    /* ── 土木の話題(第5.188節・2026-09 利用者の指定)───────────────
       ...
       **発電所は、電気の話の前に土木の話がある。** 用地・造成・基礎・
       進入路・排水がまとまらなければ、1枚のパネルも1本の鉄塔も立たない。
       **後ろへ足す。並べ替えない**(docs/notes/22 の決まり) */
    { id: 'solg_slope', label: '傾斜地の造成', hint: '崩れないか、法面、調整池、規制の強まり' },
  ],
  ene_wind: [
    ...
    /* ── 土木の話題(第5.188節・2026-09 利用者の指定)───────────────
       ...
       **発電所は、電気の話の前に土木の話がある。** 用地・造成・基礎・
       進入路・排水がまとまらなければ、1枚のパネルも1本の鉄塔も立たない。
       **後ろへ足す。並べ替えない**(docs/notes/22 の決まり) */
    { id: 'wndg_found', label: '基礎という難所', hint: '陸と洋上で、何がどう違うか' },
  ],
  ene_hydro: [
    ...
    /* ── 土木の話題(第5.188節・2026-09 利用者の指定)───────────────
       ...
       **発電所は、電気の話の前に土木の話がある。** 用地・造成・基礎・
       進入路・排水がまとまらなければ、1枚のパネルも1本の鉄塔も立たない。
       **後ろへ足す。並べ替えない**(docs/notes/22 の決まり) */
    { id: 'hydg_civil', label: '100年もつ構造物', hint: 'コンクリート、堤体の寿命。直しながら使う' },
  ],
  ene_therm: [
    ...
    /* ── 土木の話題(第5.188節・2026-09 利用者の指定)───────────────
       ...
       **発電所は、電気の話の前に土木の話がある。** 用地・造成・基礎・
       進入路・排水がまとまらなければ、1枚のパネルも1本の鉄塔も立たない。
       **後ろへ足す。並べ替えない**(docs/notes/22 の決まり) */
    { id: 'thmg_port', label: '燃料を受ける港', hint: '桟橋、防波堤、地盤沈下' },
  ],
  ene_nuke: [
    ...
    /* ── 土木の話題(第5.188節・2026-09 利用者の指定)───────────────
       ...
       **発電所は、電気の話の前に土木の話がある。** 用地・造成・基礎・
       進入路・排水がまとまらなければ、1枚のパネルも1本の鉄塔も立たない。
       **後ろへ足す。並べ替えない**(docs/notes/22 の決まり) */
    { id: 'nucg_quake', label: '地震と地盤', hint: '何を根拠に、どこまで備えるのか' },
  ],
  ene_geo: [
    ...
    /* ── 土木の話題(第5.188節・2026-09 利用者の指定)───────────────
       ...
       **発電所は、電気の話の前に土木の話がある。** 用地・造成・基礎・
       進入路・排水がまとまらなければ、1枚のパネルも1本の鉄塔も立たない。
       **後ろへ足す。並べ替えない**(docs/notes/22 の決まり) */
    { id: 'geog_road', label: '山に道路を通す', hint: '取り付け道路、法面、送電線、環境と' },
  ],
  ene_grid: [
    ...
    /* ── 土木の話題(第5.188節・2026-09 利用者の指定)───────────────
       ...
       **発電所は、電気の話の前に土木の話がある。** 用地・造成・基礎・
       進入路・排水がまとまらなければ、1枚のパネルも1本の鉄塔も立たない。
       **後ろへ足す。並べ替えない**(docs/notes/22 の決まり) */
    { id: 'gridg_civil', label: '送電線を通す土木', hint: '鉄塔、洞道、用地、住民との調整' },
  ],
  ene_batt: [
    ...
    /* ── 土木の話題(第5.188節・2026-09 利用者の指定)───────────────
       ...
       **発電所は、電気の話の前に土木の話がある。** 用地・造成・基礎・
       進入路・排水がまとまらなければ、1枚のパネルも1本の鉄塔も立たない。
       **後ろへ足す。並べ替えない**(docs/notes/22 の決まり) */
    { id: 'batg_site', label: '置き場所をつくる', hint: '離隔、防災、地盤、近所の目' },
  ],
  ene_h2: [
    ...
    /* ── 土木の話題(第5.188節・2026-09 利用者の指定)───────────────
       ...
       **発電所は、電気の話の前に土木の話がある。** 用地・造成・基礎・
       進入路・排水がまとまらなければ、1枚のパネルも1本の鉄塔も立たない。
       **後ろへ足す。並べ替えない**(docs/notes/22 の決まり) */
    { id: 'h2g_base', label: '受け入れ基地を作る', hint: '港、タンク、配管。どこに、いくらで' },
  ],
  ene_oil: [
    ...
    /* ── 土木の話題(第5.188節・2026-09 利用者の指定)───────────────
       ...
       **発電所は、電気の話の前に土木の話がある。** 用地・造成・基礎・
       進入路・排水がまとまらなければ、1枚のパネルも1本の鉄塔も立たない。
       **後ろへ足す。並べ替えない**(docs/notes/22 の決まり) */
    { id: 'oilg_port', label: '港湾という土木', hint: '浚渫、桟橋、古くなった設備' },
  ],
...
/**
 * **スピーチの場面**を返す(2026-09 利用者の指定)。
 *
 * **会話の場面は混ぜない。** あちらは相手がいてやりとりが続く場面で、
 * 1人が話しきるスピーチには当てはまらない。
 * 業界ごとの色は `industry`(窓口へ日本語で渡している)が付けるので、
 * ここは**共通の一覧だけ**にしてある。仕事と趣味だけを分ける。
 */
export const speechScenesFor = (industry) => {
  /* **業界に特化した場面も並べる**(第5.188節)。
     会話(`scenesFor`)とまったく同じ道 —— 特化 + 共通。
     登録が無い業界では `own` が空なので、**これまでどおり共通だけ**である */
  const own = ownOf(SPEECH_BY_INDUSTRY, industry)
  const isHobby = groupOf(industry) === 'hobby'
  return [...own, ...(isHobby ? COMMON_HOBBY_SPEECH_SCENES : SPEECH_SCENES)]
}
...
const ALL_SCENES = [
  ...DIALOGUE_SCENES,
  ...COMMON_HOBBY_SCENES,
  // スピーチの場面も**名前を引けるようにする**(2026-09)。
  // 入れ忘れると、教材の名前に `sp_toast` という id がそのまま出る
  ...SPEECH_SCENES,
  ...COMMON_HOBBY_SPEECH_SCENES,
  ...Object.values(SCENES_BY_INDUSTRY).flat(),
  /* **業界ごとのスピーチの場面も**(第5.188節)。
     入れ忘れると、教材の名前に `spc_safety` という id がそのまま出る */
  ...Object.values(SPEECH_BY_INDUSTRY).flat(),
]
/** すべての話題。名前を引くときに使う */
const ALL_GENRES = [
  ...READING_GENRES,
