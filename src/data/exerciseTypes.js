...
export const EXERCISE_TYPES = [
   ...
   *   > 網羅してください。
   *
   * **すぐ上の「本文に出た語句」とは別物である。**
   *
   *   ・語句 … その語の**意味と使い方**
   *   ・背景 … **なぜそう言うのか。** 日本と何が違うのか
   *
   * UNIT 1(Booking a Bus)でいえば、`departure` の意味は語句のほうだが、
   * 「アメリカは鉄道が弱く、長距離はバスが基本」「電話では社名 +
   * 自分の下の名前だけを名乗る」は背景である。
   * **意味が分かっても、これを知らないと会話が成り立たない。**
   *
   * **問いではない。** 読んで分かればよいので `question` も `answer` も
   * 持たない(**書けない欄は、最初から作らない** ——
   * ディスカッション・想定される質問と同じ考え方)。
   *
   * **読み上げは付けない**(`audioFrom: null`)。中身は日本語である。
   * ここを `prompt_en` にすると、**見出しの英語だけが読み上げられて
   * 課金される**(語句のほうと二重になる)。
   */
  {
