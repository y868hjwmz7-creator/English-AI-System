...
export const MATERIAL_KINDS = [
  ...
  /* ══════════════════════════════════════════════════════════════
     **テスト**(第5.263節・2026-09-26 利用者の指定)

       > テストは、ひとつの教材としてちゃんと作ってください。

     はじめは「作って、見て、刷るだけ」の使い捨てにしていた(第5.260節)。
     **利用者は、教材として残ることを求めていた** ——
     残らなければ、次のレッスンで開き直せないし、共有もできないし、
     ライブラリからも探せない。**この仕組みは「教材」で回っている。**

     【中身は `translate_ja_en`(和文英訳)1つだけ】
       穴埋めは**利用者の指定で廃止した**(2026-09-26)。
       日本語 → 英語だけなので、**演習の種類は足していない。**

     【`kind` を1つ足すだけ。表も列も増えない】
       0037(会議)・0043(Speech練習)とまったく同じやり方である。

     【**AI を1回も呼ばない = 0円**】
       ゲストの教材・単語帳・Quick Response 帳から、
       **もうある英文と訳を組み替えるだけ**である。
     ══════════════════════════════════════════════════════════════ */
  { id: 'test',     label: 'テスト',
    hint: 'ゲストの持ちものから組んだ、日本語 → 英語のテスト。AI は使わない',
    /* **「教材を作る」画面には出さない。** あちらは AI に書かせる画面で、
       テストは**もうあるものを組み替える**ものである
       (作る場所は、ゲストのページの「テストを作る」)。
       **さがす画面には出る** —— 作れないものも、探せなければ意味がない */
    noForm: true },
 ...
 * **「教材を作る」画面で選べる種類。**
 *
 * **テストはここから作らない**(第5.263節)—— あちらは AI に書かせる画面で、
 * テストは**もうあるものを組み替える**ものである。
 * 出すと「AI で作れる」と読めてしまう(効かない操作を見せない)。
 */
export const NEW_MATERIAL_KINDS = MATERIAL_KINDS.filter((k) => !k.legacy && !k.noForm)

/**
 * **さがすときに選べる種類。**
 *
 * **作れないものも、探せなければ意味がない**(第5.263節)。
 * ここには「テスト」も出る —— 作った本人が、次のレッスンで探せるように。
 */
export const FIND_MATERIAL_KINDS = MATERIAL_KINDS.filter((k) => !k.legacy)

/**
 * **テスト**かどうか(0069・第5.263節)。
 *
 * **画面や関数の中で `kind === 'test'` と書かない**
 * (`isDialogueKind` / `isVocabKind` とまったく同じ理由 ——
 *  置く場所の数だけ食い違う)。
 */
export const isTestKind = (kind) => kind === 'test'
 ...
 * だから**別の引き方を持っている種類は、必須にしない。**
 *
 *   ・記事・会話・会議・スピーチ … ジャンル・場面・見出しで引ける(0007 からの決まり)
 *   ・**テスト** … 中身はそのゲストの持ちものそのもので、
 *     弱点で引くものではない。誰の・いつのテストかは教材名に入っている
 *
 * **判断はここ1か所。** `createMaterial()` の中で
