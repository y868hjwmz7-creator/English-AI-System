 ...
 *
 * ============================================================================
 * 【トレーナーには 35 冊ぜんぶ出す】(第5.246節・2026-09-23 利用者の指定)
 *
 *   > トレーナーの単語帳の冊選択のタブには常に入れておきたいですが、
 *   > まだ中身のないものは表示されないようにしましょう。
 *
 *   もとは**トレーナーも「出された冊だけ」**だった(2026-09)。
 *   作った本人が、自分に出す操作をもう1回しないと開けない。
 *
 ...
 * 【中身の無い冊は出さない】
 *
 *   35 冊のうち、まだ1語も作っていない冊が開けてしまうと、
 *   **開いた先が空**になる。**行き止まりを作らない**(CLAUDE.md)。
 *
 *   **数が分からないときは、落とさない**(`counts` が `null`)。
 *   0 と「数えられなかった」を取り違えない —— 通信が届かないだけで
 *   冊が消えると、**あるはずのものが黙って無くなる。**
 *
 * @param who.features その人に出ている冊(`learner_features`)
 * @param who.all      true なら features を見ずに 35 冊ぜんぶ(トレーナー)
 * @param who.counts   冊ごとの語数(`loadShelfCounts()`)。
 *   **渡さなければ、数では絞らない**
 * ============================================================================
 */
