...
/**
 * ============================================================================
 * **読み名簿 —— 大文字の並びでも、文字ずつ読まない名前**(第5.269節)
 *
 ...
 *   > 指定するべきなのですか?
 *
 * ── **1文字ずつに開いていたのは、こちらのコードだった** ──────────
 *
 *   読み上げの前に `speakText()` が
 *
 *       We visited UMITO.  →  We visited U M I T O.
 *
 *   と**書き換えていた。** 知らない大文字の並びは文字ずつ読むのが
 *   既定で(`IBM` `ATM`)、それ自体は正しい。
 *   **ここに名前を登録すると、その1語だけが例外になる。**
 *
 * ── **「細かい指定」に書かなくても効く** ─────────────────────
 *
 *   第5.266節では教材ごとに書いてもらう形にしたが、
 *   **会社の名前は毎回出る。** 毎回書かせるのは作りのほうが悪い。
 *   ここに1回登録すれば、**すでに発行した教材にも効く**
 *   (読み上げの直前に通るので、作り直さなくてよい)。
 *   教材ごとの一度きりの名前は、これまでどおり細かい指定に書ける。
 *
 * ── **推測で足さない** ────────────────────────────────
 *
 *   読み方は**利用者に訊いてから**足す。
 *   `UMITO` は「ウミト」と教わった(2026-09-26)。
 *   英語の読み上げに渡す綴りは `kanaSpell.js` の作法と同じ
 *   (`ウ` → oo / `ミ` → mee / `ト` → toh)。
 *
 * ── **一覧は減らさない**(共通ルール)──────────────────────
 *
 *   減らすと、読めていた名前が読めなくなる。
 *
 * **足すと、その名前を含む英文の音声は1回だけ作り直しになる**
 * (指紋が変わるため)。**いま読み方が間違っている文だけ**なので、
 * これは払ってよい費用である(`audioClips.js` の同じ決まり)。
 * ============================================================================
 */
export const SAY_AS_BOOK = {
  /* 利用者の会社。**「ウミト」**(2026-09-26 に本人へ確認) */
  UMITO: 'Oo-mee-toh',
}
