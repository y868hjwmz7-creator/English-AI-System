...
/**
 * **業種べつの単語帳(棚)で使う段。8つ**(2026-09 利用者の指定)。
 *
 ...
 * 【なぜ 14 段のままにしないか】
 *
 *   14 段は**教材とゲストの物差し**である。棚に並ぶのは1語ずつなので、
 *   「この語は B1 か B1+ か」は誰にも決められないし、
 *   絞り込みのプルダウンに 14 個並ぶだけになる。**8つで足りる。**
 *
 * 【別の表を作らない】
 *
 *   8つは**どれも `CEFR_LEVELS` にある id** である。だから名前も GSE も
 *   `cefrOption()` がそのまま出せる(`A1 (GSE 22-25)` など)。
 *   **ここには id の並びしか置かない** —— 表を2つ持つと、
 *   GSE を直した日に片方だけ古くなる。
 *
 * 【窓口にも同じ一覧がある】
 *
 *   `generate-material` の道具(`emit_shelf_words`)の `level` が、
 *   同じ8つを enum で持っている(窓口からは `src/` を読めない)。
