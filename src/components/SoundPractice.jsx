/**
 * 単語とフレーズの音 — **いまは出していない**(2026-09 利用者の指定)。
 *
 *   > 発音の機能は一度廃止してください。
 *   > いつでも戻せるように
 *
 * ============================================================================
 * 【この部品は生きている。呼ばれていないだけ】
 *
 *   出すかどうかは **`soundPracticeOn()`(`src/data/soundPractice.js`)1か所**が
 *   決めており、いまは閉じている。**戻す日はあの1行を `true` にするだけ**で、
 *   ここは1文字も書き直さなくてよい。
 *
 *   中身は `PronunciationPractice.jsx` にあったものを**そのまま**移してある。
 *   **見た目も振る舞いも1つも変えていない**(寄せたついでに直さない)。
 *
 * 【なぜ「畳んで隠す」ではなく、部品に切り出したのか】
 *
 *   JSX を `{…&& (…)}` で囲むだけだと、**材料を取りに行く `useEffect` は
 *   そのまま走る。** 画面を開くたびに、誰も見ない一覧のために
 *   教材をぜんぶ読みに行くことになる。
 *   部品ごと描かなければ、問い合わせも起きない。
 *
 * 【いまは「器」だけ】(2026-08 利用者の指定・廃止前からの決まり)
 *   採点は入れていない。いまある発音スコア(`pronunciation.js`)は
 *   **録音を解析していない仮の数値**で、実力を測るものではない。
 *   仮の点数を看板にすると、**測れていないものを測れているように見せる**
 *   ことになる。だから置くのは、練習に要るものだけである。
