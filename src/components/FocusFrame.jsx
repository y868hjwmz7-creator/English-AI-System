    ...
    <div className={`focus${plain ? ' focus--plain' : ' focus--sheet'}`
            ...
            {hasMore && (
              <>
                ...
                <div id="focus-settings"
                  ...
                  {tools}
                  {settings}
                  {/* ── **いま見ている版**(2026-09 実機・利用者の指定)───────
                      集中モードは画面をまるごと覆うので、**フッターの版が
                      見えない。** 「直したはずのものが直っていない」の多くは
                      **端末に古い内容が残っていた**だけなので、
                      ここで確かめられるようにする
                      (共通ルール「版が分かるようにしてから確認を依頼する」)。

                      **帯を2行にしてまでは出さない**(利用者の指定
                      「再生プレーヤーが2行になるのは絶対にダメです」)。
                      だから**畳んだ「表示」を開いたときだけ**出す ——
                      パソコンは集中モードを閉じれば、すぐ下に出ている */}
                  {import.meta.env.VITE_BUILD_STAMP && (
                    <span className="focus-ver">版 {import.meta.env.VITE_BUILD_STAMP}</span>
                  )}
                </div>
              </>
            )}
      ...
      <div className="focus-barwrap">
        {/* **紙を持たない集中モードには出さない。** 閉じるのは
            ...
            **下の帯が無いときも出さない**(第5.250節・2026-09-23 利用者の指定)。

              > スマホから教材内の quick response に入ると
              > 下の「集中モードを出る」というボタンはいらなくないですか？
              > 集中モードと、わざわざ銘打つ必要がないからです

            このボタンは**帯の上に浮かせる**作り(`bottom: 100%`)である。
            **浮かせる先が無ければ、いちばん下に落ちて中身に重なる** ——
            教材の中の Quick Response は帯を渡さないので、実際に
            「言えた」の上へかぶさっていた(利用者の写真)。
            左上の ✕ が同じ場所へ戻すので、**同じことをするものが2つ**でもある。

            **帯を渡している画面(読む・6Steps・レッスン・スピーチ)は
            これまでどおり出る。** 言われたのは、この画面である */}
        {plain || !bar ? null : (
