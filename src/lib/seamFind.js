...
/**
 * **これ以上ずれていたら、対が違う**(38手め)。
 *
 * 控えの継ぎ目と、測った静けさが 0.3 秒より離れていたら、
 * それは**別の継ぎ目**を見ている。当てずっぽうで結ばない。
 */
 ...
 *
 *   > 文によりほんの一瞬ですが最後が切れ、前の文の最後の一瞬が
 *   > 入ってしまいます。。。much で終わるところが mu しか入らない
 *
 ...
 * ## なぜ「文によって違う」のか
 *
 * `alignment` は**文字に時間を割り当てたもの**であって、音が鳴り終わった
 * ところではない。ずれる量は**語尾の音の種類**で変わる。
 *
 * | 語尾 | 控えの `end` と、実際に音が消えるところ |
 * |---|---|
 * | `much` `stopped` `desk`(破裂音・摩擦音) | **50〜150ms ずれる** |
 * | `day` `time` `on`(母音・鼻音) | ほぼ一致する |
 *
 * **だから一律の数を足せない。** 足せば後者が次の声を踏む
 * (37手めで一律 `TAIL_KEEP` を足して取り下げた。利用者の予想どおり)。
 *
 * ## 測る。**しかも、今日より早くならないことを形で保証する**
 *
 *   新しい終わり = max(控えの end, 測った静けさの入口)
 *   新しい頭     = max(控えの start, 測った静けさの出口)
 *
 * **`max` で今日の値を下限にしてあるので、いまより早く切れることは
 * 構造的に起こりえない。** 測れなかった継ぎ目は `null` を返し、
 * 呼ぶ側が今日の値をそのまま使う ——**1ミリ秒も変わらない。**
 *
 * ## 間の無い継ぎ目は、測らない(`null`)
 *
 * 文と文が地続きに鳴っているところは、`speechRuns()` が1つの run に
 * するので、静けさの対が見つからない。**それが正しい** ——
 * そこには延ばせる余地が無い(延ばせば次の文を飲み込む)。
 *
 * @param {Array<{start:number,end:number}>} spans 控えの区間(文)
 * @param {Array<{from:number,to:number}>} runs 測った声のところ
 * @param {number} [cap] これ以上離れた静けさは結ばない
 * @returns {{tail:Array<number|null>, head:Array<number|null>, hit:number}|null}
 */
export function seamEdges(spans, runs, cap = EDGE_CAP) {
  if (!Array.isArray(spans) || spans.length < 2) return null
  if (!Array.isArray(runs) || runs.length < 2) return null
  const tail = new Array(spans.length).fill(null)
  const head = new Array(spans.length).fill(null)
  let hit = 0
  for (let i = 0; i < spans.length - 1; i += 1) {
    const e = Number(spans[i].end)
    const s = Number(spans[i + 1].start)
    if (!Number.isFinite(e) || !Number.isFinite(s)) continue
    const mid = (e + s) / 2
    /* 静けさ(run と run のあいだ)のうち、**控えの継ぎ目にいちばん近いもの**。
       発言の途中の息継ぎも静けさだが、あちらは控えの継ぎ目から離れている */
    let best = null
    for (let k = 0; k < runs.length - 1; k += 1) {
      const a = Number(runs[k].to)
      const b = Number(runs[k + 1].from)
    ...
    if (!best) continue
    /* **ここは測った値をそのまま返す。** 「今日より早くしない」の歯止めは
       `applyEdges()` **1か所**が持つ —— 2か所に置くと、片方を外しても
       もう片方が拾ってしまい、**検証が壊れたことに気づけない**
       (実際にそうなって、赤チェックが赤くならなかった) */
    tail[i] = Math.min(best.a, best.b)
    head[i + 1] = best.b
    hit += 1
  }
  return hit ? { tail, head, hit } : null
}

/**
 * 波をほどいて、**文の継ぎ目の本当の縁**を出す(38手め)。
 *
 * `measureSeams()` と**同じ道具・同じ梯子**を通す(`speechRuns()`)。
 * ちがうのは、返すのが「ずれ」ではなく**縁そのもの**という点だけである。
 *
 * @returns {{tail:Array<number|null>, head:Array<number|null>, hit:number, q:number}|null}
 */
export function measureEdges(samples, rate, spans) {
  const rms = frameRms(samples, rate)
  if (!rms.length) return null
  for (const q of TRY_RATIOS) {
    const runs = speechRuns(rms, HOP_SEC, { level: quietLevel(rms, q) })
    const got = seamEdges(spans, runs)
    if (got) return { ...got, q }
  }
  return null
}
