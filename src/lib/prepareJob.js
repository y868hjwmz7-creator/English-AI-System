...
/**
 * ============================================================================
 * **うまくいった知らせは、ひとりでに閉じる**(第5.265節・2026-09-26 実機)
 *
 *   > 教材が完成して発行した後もこの帯がしつこく残ります。
 *   > これを改善してください。
 *
 * 支度は**発行のあとに始まる**ので、終わったときにはもう別の画面にいる。
 * ところが帯は「閉じる」を押すまで残り、**どの画面のいちばん上にも
 * 貼り付いたまま**だった。
 *
 * 【失敗は消さない】
 *   **成功と失敗を、同じ見た目で終わらせない**(CLAUDE.md)。
 *   理由が出ているものを勝手に消すと、**何が起きたのか誰も知らないまま**に
 *   なる。消えるのは「ぜんぶ済んだ」ときだけである。
 *
 * 【費用の数字は、消える前に読めるだけ置く】
 *   帯には**何本作ったか(= 課金)**が出ている(「うち新しく 13 本・420 文字」)。
 *   **見えない費用は管理できない**ので、すぐには消さない。
 *   20 秒は、別の画面へ移ってから目を上げても間に合う長さである。
 *
 * 【次の支度が始まったら、その時点で消える】
 *   `id` を見ているので、**次の教材の知らせを消してしまうことはない。**
 * ============================================================================
 */
export const PREP_HIDE_MS = 20000

/** 済んだ知らせを、あとで閉じる。**失敗は閉じない** */
function hideLater(id) {
  setTimeout(() => {
    if (task?.id !== id) return          // 次の支度が始まっている
    if (task.state !== 'done') return    // まだ走っている
    if (task.error) return               // **失敗は消さない**
    clearPrepare()
  }, PREP_HIDE_MS)
}
