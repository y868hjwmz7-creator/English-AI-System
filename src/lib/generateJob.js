 ...
 *
 * 仕事の状態は**書類(モジュール)に1つだけ**置いてあるので、
 * **画面を読み込み直すまで消えない。** そのためトレーナーで教材を作ったあと、
 * そのままゲストで入り直すと、**前の人の教材の名前が残る。**
 *
 * ・**`clearJob()` とは別に用意する。** あちらは「結果を受け取ったあと」に
 *   呼ぶもので、走っている最中には呼ばれない前提である
 * ・**通信そのものは取り消さない**(送った1回はどのみち課金されている)。
 *   `cancelled` を立てておけば、段と段のあいだで止まる
...
export function forgetJob() {
  if (!job) return
  if (job.state === 'running') job = { ...job, cancelled: true }
  job = null
  emit()
}
