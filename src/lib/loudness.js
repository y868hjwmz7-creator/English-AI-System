 ...
 * 前の直しでは「差し替える前に `volume = 0`」にしたが、
 * **0 に跳ぶこと自体が段差**なので、鳴る場所を移しただけだった。
 *
 * 音量を**時間をかけて**上げ下げすれば、段差そのものが無くなる。
 * 動かすのは、もともと使っている `<audio>` の `volume` だけである
 * (**音の通り道は変えない** — このファイルの冒頭の失敗をくり返さない)。
 *
 * **決め方はここ1か所。** `audioClips.js` に数字を書くと、
 * 手元では一度も確かめられない(あちらは Supabase を引き連れる)。
 * ここに置けば `npm run test:audio` で機械的に見張れる。
 * ========================================================================= */

/**
 * **何ミリ秒おきに音量を動かすか。**
 *
 * **画面の描き替え(`requestAnimationFrame`・約 16ms)では粗すぎた**
 * (2026-09 実測)。1.5 秒の音を本物の `<audio>` で鳴らして測ったところ、
 *
 *   ・鳴り始め … 1ms の次が 42ms。**途中が1段しかない**
 *   ・鳴り終わり … 最後のコマが「残り 41ms」で、
 *     **音量 0.205 のまま終わっていた**(0 まで届いていない)
 *
 * つまり**半分の高さから急に切れて**いた。これでは「プチッ」は消えない。
 * だから `setInterval` で**10ms ごと**に動かす。
 */
export const FADE_STEP = 10

/**
 * 鳴り始め。**短く** — 頭の子音を削らない。
 *
 * ただし**短すぎると、1刻みめの持ち上がりが大きくなる。**
 * 実測では最初の一刻みが「17〜21ms」に来るので、50ms では
 * いきなり音量の 34% まで上がっていた。70ms なら 3 割を切る。
 */
export const FADE_IN = 70
/**
 * 鳴り終わり。**長めに取る** — ここが「プチッ」の出どころだった。
 * 10ms 刻みで12段。最後の1段でも音量は元の 8% ほどまで下がっているので、
 * そこで音が終わっても段差にならない。
 */
export const FADE_OUT = 120
/**
 * **終わりの、この手前で 0 にする**(2026-09 実測)。
 *
 * 10ms 刻みにしてもなお、**最後の一刻みは「残り 33ms」で終わって**いた
 * (音量 0.11 のまま音が切れる)。時計の揺らぎと、`ended` が
 * 長さぴったりでは来ないためである。
 *
 * だから**終わりを少し手前に見立てる。** そこで 0 に届かせ、
 * 残りは音量 0 のまま鳴らし切る。**切っているのではない** —
 * 最後まで鳴らしたうえで、そこだけ聞こえなくしている。
 */
export const FADE_TAIL = 25

/** 止めたとき。押した手応えが遅れない範囲で、いちばん長く取る */
export const FADE_STOP = 60

/**
 * いま入れるべき音量。
 *
 * @param {number} gain  その声の音量(`gainFor`)
 * @param {number} inMs  鳴り始めてから何ミリ秒か(**実際の時間**)
 * @param {number} outMs 鳴り終わりまで何ミリ秒か(**実際の時間**)
 * @returns {number} 0〜gain
 *
 * **速さで割ってから渡すこと。** `currentTime` は音声の中の時間なので、
 * 1.2 倍で鳴らしていれば実際の時間はその 1/1.2 で過ぎる。
 *
 * **短い音では、入りと終わりが重なる。** そのときは
 * **小さいほうを採る**(足し合わせない)。重なったまま両方かけると
 * 1 を超えたり、真ん中で持ち上がったりする。
 */
export function fadeGain(gain, inMs, outMs) {
  const g = Number(gain)
  if (!Number.isFinite(g) || g <= 0) return 0
  let v = g
  if (Number.isFinite(inMs) && inMs < FADE_IN) {
    v = Math.min(v, g * (Math.max(inMs, 0) / FADE_IN))
  }
  /* **終わりの手前で 0 に届かせる**(`FADE_TAIL`)。
     ここを引かないと、最後の一刻みが音量を持ったまま音が終わる */
  const out = Number.isFinite(outMs) ? outMs - FADE_TAIL : outMs
  if (Number.isFinite(out) && out < FADE_OUT) {
    v = Math.min(v, g * (Math.max(out, 0) / FADE_OUT))
  }
