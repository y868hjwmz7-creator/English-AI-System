 ...
 *
 *   頼む先を `T` とすると、実際に鳴り出すのは最悪 `T − 1枚` である。
 *   だから **`T − 1枚 ≥ 間の始まり`** を満たせばよい。
 *
 *     間が 46ms 以上ある … これまでどおり `頭 − SEEK_LEAD`(1ミリも変えない)
 *     間が足りない       … 足りないぶんだけ**うしろへ逃がす**
 *     間がまったく無い   … `頭 + 1枚`(**26ms だけ頭が欠ける**)
 *
 *   **欠けるのは、その文自身の 26ms である。**
 *   前の文の最後が入るより、そのほうがましである(利用者の
 *   「一瞬といえど違和感は非常に大きい」は**前の声**についての言葉)。
 *   **1枚より深くは欠かせない**(`頭 + 1枚` で頭打ち)。
 *
 *   **推測を1つも足していない。** フレームの大きさは決まっており、
 *   間の長さは控えが言っている。**当てずっぽうの補正ではない。**
 *
...
export function landSec(start, gap) {
  ...
  // 吸い寄せられても間の中に着く、いちばん手前
  const floor = s - g + FRAME_SEC
  // 欠けてよいのは、多くてもフレーム1枚ぶん
  const ceil = s + FRAME_SEC
...
function landEdge(list, i) {
  ...
  /* **いちばん最初だけは、逃がさない。** 前に声が無いのだから、
     手前に吸い寄せられても入ってくるものが無い(頭の無音である)。
     ここで逃がすと、頭から鳴らし直すたびに 26ms 欠ける */
  if (i <= 0) return s
  const p = Number(list[i - 1]?.end)
