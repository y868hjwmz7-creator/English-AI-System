...
const staleNote = () => (clipFnStale()
  /* **いちばん効くものを、先に書く**(2026-09 実機・33手め)。
     もとは v3 と声の指定しか挙げていなかったので、
     **文ごとのリピートがずれる原因**がここから読み取れなかった。
     利用者に「先にこれをやらないといけないのか」と訊かせている。 */
  ? '読み上げの窓口(speak)が古いため、'
    + '発言ごとの区切り(文ごとのリピート・ハイライトの精度)と、'
    + 'ElevenLabs の v3、声の細かい指定(訛りの強さ・雑音の出やすさ)が'
    + '反映されていません。'
    + `いま置かれているのは ${fnRev}、必要なのは ${NEED_FN_REV} 以降です。`
    + ' Supabase → Edge Functions → speak を置き直してから、'
    + 'その教材の「読み上げ音声を作り直す」を1回押してください。'
  : null)
...
/**
 * **1本にまとめられなかった理由を知らせる**(2026-09 実機・11手め)。
 *
 *   > これではなぜ一本にならなかったのかが分からないままなので、
 *   > また発言ごとになってしまった教材があれば同じことが起こる。
 *   > **根本的に解決ではないですよね**
 *
 * 理由は `wholeNote` に**必ず入っていた**のに、読んでいるのは
 * 支度の帯だけで、**鳴らしたときには誰も出していなかった。**
 * だから「なぜこの教材だけ違うのか」を、こちらも利用者も
 * **知りようがなかった。**
 *
 ...
 */
/**
 * **着いた先が、頼んだ秒よりこれだけ手前なら直す**(秒・29手め)。
 *
 * もとは `SEEK_MISS`(78.6ms)と**同じ値に括り付けて**あった。ところが
 * `landSec()` があらかじめ 78.6ms 倒していたので、**その先へは行かない** ——
 * **この見張りは一度も働いていなかった。** 残ったのは頭の欠けだけである。
 *
 * **直しているあいだ、音は1ミリ秒も出ない**(`muted` にして止めてある)。
 * だから**小さく取ってよい。** フレーム1枚の吸い寄せも、ここで直す。
 */
const LAND_EPS = 0.008
  ...
  return new Promise((resolve) => {
    ...
    const tick = () => {
      ...
      /* **長さも一緒に渡す。** 1本にまとめられなかった教材では、
         ...
         **`|| 0` にしない**(2026-09 実機・37手め)。メタデータを
         読み終えるまで `duration` は `NaN` で、それを 0 にして渡すと
         受け取った側が「**長さがそろっている**」と読む ——
         合わせが素通りして、**初回だけハイライトが先に進む。**
         **分からないものは、分からないまま渡す**(`knownDur()` が読む) */
      onTime?.(Number(el.currentTime) || 0, Number(el.duration))
