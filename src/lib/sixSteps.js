 ...
 *
 * ── ここから下は、**画面から `step === '…'` を消すために足した列**
 *    (第5.239節・2026-09 利用者の指定「必要なアプリの挙動はほとんど同じ」)。
 *
 ...
 *    **本当に違うのは、次の3つだけ**だった。
 *
 * @property {'dictation'|'slash'|'sentence'|'passage'} view
 *   どの練習の形か。**画面はこれで振り分ける。**
 *   7番目を足すときも、ここに1つ書けば画面は1行も変わらない
 * @property {boolean} barRate
 ...
 *   集中モード(1つずつ画面に固定する)に意味があるか。
 *   **② だけ false**(2026-09-22 利用者の指定)——
 *
 *     > スラッシュリーディングの段階で集中モードは必要ないので排除で良い
 *
 *   ② は**自分で区切りを入れていく**作業で、1つずつ送る形にしても
 *   やることは変わらない。しかも「文章全体」を選んでいると**1つしか
 *   無い**ので、送るものが無い。**効かない操作を見せない**(CLAUDE.md)
 */
export const SIX_STEPS = [
  ...
  },
  {
    ...
    // **少し遅れて追いかける**から、耳で確かめてからまねられる。
    how: [
      '本文を**見ながら**、お手本を**2〜3語うしろから**追いかけて読む。',
      '**同時に声を出さない。** 聞こえた音を確かめてから、まねる。',
      'まず**音だけ**をまねる。',
...
/**
 * **その取り組み方に、集中モードがあるか**(2026-09-22 利用者の指定)。
 *
 *   > スラッシュリーディングの段階で集中モードは必要ないので排除で良い
 *
 * **判断はここ1か所。** ボタンを出す場所は2つある
 * (レッスン表示の上のボタンの行 / ゲストの宿題の中)ので、
 * 画面ごとに書くと必ず食い違う(CLAUDE.md)。
 *
 * **知らない id のときは、`stepOf()` と同じものを見る**(= ①)。
 * 控えに古い id が残っていると画面は ① を描くので、
 * **ボタンだけ別の答えを返すと、描かれているものと食い違う。**
 */
export const canFocusStep = (id) => stepOf(id)?.focus === true

/**
 * **集中モードで1つずつ送るとき、何を数えるか。**
 *
 ...
 *
 * @param step いまの取り組み方
 * @param 並び `{ dictation, slash, passage, sentence }`(呼ぶ側が用意する)
 */
export const focusListOf = (step, 並び) => {
  const v = stepOf(step)?.view
  if (v === 'dictation') return 並び.dictation ?? []
  if (v === 'slash') return 並び.slash ?? []
  if (v === 'passage') return 並び.passage ?? []
  return 並び.sentence ?? []
}

/**
 * **何を「1つ」と数えるかの言葉**(文 / 段落 / 発言 / 文章)。
 *
 * 記事は段落、会話・会議は発言(`bodyUnitWord()` と同じ決まり)。
 *
 * 【`'all'` は、どこにも無い値である(**直していない**)】
 *
 *   画面にあった判断を、そのままここへ移した。そのとき気づいたが、
 *   ② の単位の id は `para` / `whole` の2つしかない(`SLASH_UNITS`)。
 *   **`'all'` はどこからも渡らないので、「文章」には一度もならない。**
 *   「文章全体」を選んでいても、集中モードの帯には「段落」と出る。
 *
 *   **寄せたついでに直さない**(CLAUDE.md)。見た目が変わるので、
 *   利用者に報告してから直す。ここでは**そのまま持ってきてある。**
 */
export const focusUnitWord = (step, isDialogue, slashUnit = 'para') => {
  const v = stepOf(step)?.view
  if (v === 'slash') return slashUnit === 'all' ? '文章' : bodyUnitWord(isDialogue)
  if (v === 'passage') return bodyUnitWord(isDialogue)
  return '文'
}
...
export const SLASH_UNITS = [
  { id: 'para', label: '段落ごと', hint: '段落ごとに区切る' },
  { id: 'whole', label: '文章全体', hint: '本文をまるごと1つとして区切る' },
]

/**
 * **会話・会議では「発言ごと」と書く**(2026-09 実機・利用者の指定)。
 *
 *   > これは、会話だから「段落ごと」ではなく「発話ごと」で分けてほしいです。
 *   > 会議も同じく。記事だと段落ごとです。
 *
 * 記事は**段落**、会話・会議は**発言**で数える、というのは
 * もともとの決まりである(`countUnit()`)。ここだけ「段落ごと」と
 * 決め打ちにしていたので、会話でも「段落ごと」と出ていた。
 * **単位の言葉は、教材の形から決める。**
 *
 * 会議も本文の演習は `dialogue` なので、これ1つで両方に効く。
 */
export const slashUnitsFor = (isDialogue) => (isDialogue
  ? [
    { id: 'para', label: '発言ごと', hint: '1つの発言ごとに区切る' },
    { id: 'whole', label: '会話全体', hint: '会話をまるごと1つとして区切る' },
  ]
  : SLASH_UNITS)

/**
 * 本文のひとかたまりを、日本語で何と呼ぶか(**段落 / 発言**)。
 *
 * 2026-09 利用者の指定「『発言』にしましょう」。
 * `slashUnitsFor()` で選択肢の名前は直したが、**訳の札(「段落の訳」)と
 * 通し表示の「段落で区切らない」が、まだ「段落」のまま**だった。
 * 記事は**段落**、会話・会議は**発言**である(`countUnit()` と同じ決まり)。
 *
 * **1か所で決める。** 出す場所は4つ(スラッシュリーディング・
 * ディクテーション・1文ずつ・通し表示)あり、書き写すと必ずどこかが残る。
 */
export const bodyUnitWord = (isDialogue) => (isDialogue ? '発言' : '段落')
