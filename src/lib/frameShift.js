...
export function shiftTrainings(done = null) {
  ...
  /* **入れ替えは、ここに1枚として出さない**(2026-09 利用者の指定
     「どうするのが学習者が一番使いやすく、仕組みを理解しやすいでしょうか」)。

     もとは「④ 入れ替えて数をこなす」という**別の入口**だった。
     すると「させる」を鍛えたい人は、**3か所を回る**ことになる ——
     ①1組(言い直す9問)・④(入れ替える86問)・⑤(見分ける27問)。
     **その3つが同じ型の練習だとは、画面のどこにも書いていなかった。**

     いまは**組の中の「段2」**である。組を1つ押せば、
     見分ける → 入れ替える → 言い直す が**いつも同じ順で**並ぶ。 */
  return out
}
