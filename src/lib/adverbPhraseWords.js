...
import { adverbPhraseRows } from '../data/adverbPhrases.js'
...
export const ADVERB_PHRASE_SEEN_LIMIT = 5000
 ...
 */
export async function loadAdverbPhraseWordbook({ learnerId = null } = {}) {
  const day = todayKey()
  if (!supabase) return ok(adverbPhraseRows([], { today: day }))
  ...
  if (!who) return ok(adverbPhraseRows([], { today: day }))

  const { data, error } = await supabase
    .from('word_reviews')
    .select('word_norm, status, box, due_on, learn_streak, added_at, updated_at')
    .eq('learner_id', who)
    .limit(ADVERB_PHRASE_SEEN_LIMIT)
  if (error) return ng(error)

  return ok(adverbPhraseRows(data ?? [], { today: day }))
}
