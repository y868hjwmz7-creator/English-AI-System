...
合計見積: **約 120pt**（1 人開発なら 5〜6 か月、2 人なら 3 か月程度が目安）

---

## EPIC-0. 前提確認とスパイク（14pt）— **ここから始める**

実装ではなく「調べる」フェーズ。**ここを飛ばすと後で作り直しになる。**

| # | ストーリー | 受入条件 | pt |
|---|---|---|---|
| 0-1 | 法務確認の最優先 2 件を実施 | [09](09-LEGAL-CHECKLIST.md) の L-0（古物商の線引き）と L-8（リチウム電池）に結果と日付が記入されている | 3 |
| 0-1b | **古物商許可の申請を出す** | 管轄警察署に申請済み。ドメインを取得し URL 届出に含めた（[11](11-SETUP-CHECKLIST.md) §3 A-1） | 2 |
...
| M5 | 「送れる」 | EPIC-7, 8 完了 | **自分の持ち物**を 1 件、海外へ発送して着荷する |
| M6 | 「α 完了」 | EPIC-9, 10 完了 | 3 件の売却で REAL NET PROFIT が確定する |
| M7 | 「HUNT 解禁」 | 古物商許可の取得後 | 店頭で仕入れ、実際に転売して利益が出る |
...
## 4. 検証の順序（古物商許可が未取得であることを踏まえて）

```mermaid
flowchart LR
    A["古物商許可を申請<br/>（40日前後かかる）"] -.並行.-> B["M1〜M4<br/>開発"]
    B --> C["M5-M6<br/>自分の持ち物を売って検証<br/>★許可を待たずにできる"]
    A --> D["M7<br/>HUNT の実地検証<br/>（許可取得後）"]
    C --> D
```

**MVP の機能は変えない。検証に使う商品を変えるだけである。**
自分の持ち物（使わなくなったカメラなど）で M5・M6 を通せば、
越境販売の一本道は許可なしで検証できる。
...
1. **管轄の警察署に、古物商許可が必要になる線引きを確認する**（0-1）
2. **ドメインを決めて取得し、古物商許可を申請する**（0-1b）
3. eBay の Developer 登録と、カメラ 20 型番のデータ取得を試す（0-2）
4. 実写 50 点で AI の商品特定精度と原価を測る（0-4）
5. 並行して `value-engine` の型とテストを書き始める（2-1, 2-3）

**1 と 2 は時間がかかるので先に出す。3〜5 はその待ち時間に進める。**
これが終われば、設計の前提が実測に置き換わる。そこで本設計書を改訂し、実装フェーズに入る。

生活と手続きの段取り全体は [11-SETUP-CHECKLIST.md](11-SETUP-CHECKLIST.md) を参照。
