...
**決定済み（D-2）**: クライアントは **Expo (React Native)** とする。

### 4.1 Expo 側の具体構成

| 用途 | 採用 | 理由 |
|---|---|---|
| フレームワーク | **Expo SDK（managed workflow）** | ネイティブのビルド環境を自前で持たずに済む。カメラ・通知・セキュア保存が公式モジュールで揃う |
| 画面遷移 | **expo-router**（ファイルベース） | 画面数が多い（[02](02-SCREENS.md) は 40 超）。構造で整理できる |
| カメラ | **expo-camera** + バーコード検出 | バーコードは商品特定の第 1 優先手段（[05](05-AI-PIPELINE.md) §4） |
| 画像の前処理 | **expo-image-manipulator** | 端末内でのリサイズ・回転。送信量を減らす |
| 秘匿保存 | **expo-secure-store** | セッショントークン（Keychain / Keystore） |
| ローカル保存 | **expo-sqlite** | オフライン時の撮影キューと市場スナップショットのキャッシュ（§7） |
| 通知 | **expo-notifications** | SELL ALERT（V1） |
| サーバー状態 | **TanStack Query** | 取得・キャッシュ・再試行。オフライン復帰時の再送と相性がよい |
| 端末内状態 | **Zustand** | 軽量。撮影中の一時状態や設定に使う |
| フォーム・検証 | **React Hook Form** + **Zod** | Zod のスキーマを API と共有できる |
| 配布 | **EAS Build / EAS Update** | 実機テストと、審査を挟まない小修正の配信 |
| テスト | **Jest** + **React Native Testing Library** | `value-engine` は Jest のみで完結する |

> **開発初期は Expo Go で動かせる**ため、Apple / Google の開発者アカウントがなくても
> 開発を始められる。登録が要るのは実機での本格検証以降（[11](11-SETUP-CHECKLIST.md) §6）。

### 4.2 Web 版について

MVP では作らない。ただし Expo は Web 出力を持つため、
将来「PC で出品文を編集したい」という要求が出たときに、
画面を作り直さずに対応できる余地は残す（`ui` パッケージを分けている理由）。

`要確認`: Supabase を採用するかは最終確認が必要（既存プロジェクトでの運用知見があるため推奨）。
