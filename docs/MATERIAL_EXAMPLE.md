...
## リーディングとダイアローグについて(2026-08 追加)

この文書の手本は**文型ドリル**のものである。
リーディング(記事)とダイアローグ(会話)は別の作りになる。
決まりは仕様書 第5.17節にある。要点だけ:

- **練習問題ではなく読み物である。** 前を受けて話が進み、最後に区切りがつく
- 記事は6段落・250〜350語、会話は14発言。**短い文の寄せ集めにしない**
- 具体的な事実・数字・その業界ならではの細部を入れる。
  「テクノロジーは私たちの生活を変えています」で埋めない
- 会話は**場面で話し方を変える**。噂話と交渉で同じ言い方にしない
- 内容理解と語句は、**できあがった本文を渡してから**作る
