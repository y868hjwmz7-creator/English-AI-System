...
--   0058 … その棚を、**独立した単語帳**にする(棚の中だけで練習でき、
--          覚え具合も棚の側に残ります。自分の単語帳には1語も入りません)
--   0059 … トレーナーが、自分自身にも業種べつの単語帳を出せるようにする
--   0060 … 苦手タグに「無生物主語」と「名詞構文」を足す
--   0061 … 苦手タグの「表現」に「丁寧な言い回し」を足す
--   0062 … Quick Response の復習を、1回に何問まで返すか
--   0063 … 演習の種類に「文化の背景」を足す
--          (演習そのものは 2026-09 に廃止しましたが、**値は消しません** ——
--           狭めると、その値が入っている DB に貼り直せなくなります)
--   0064 … 教材に「どの冊の、第何 UNIT か」を控える
--   0065 … 本文から拾った「かたまり」に、分類・本文の文章・練習を持たせる
--   0066 … Quick Response の復習に「冊」を持たせる(覚えておきたい表現集)
...
--   ・表が1つ増えます(`shelf_reviews`・0058)。**空から始まります。**
--     棚の語の覚え具合です。自分の単語帳とは別に残ります。
...
--    **引数を取らない**ので、画面の SetupStatus が
--    「0059 を貼ったか」の印としてそのまま呼べる(0056 と同じ考え方)。
-- ────────────────────────────────────────────────────────────────
...
-- 0063 … 演習の種類に「文化の背景」を足す
--
--   RIZAP ENGLISH の教材を取り込むための下ごしらえ(2026-09 利用者の指定)。
--   語句(`vocab_note`)とは別物で、**なぜそう言うのか・日本と何が違うのか**
--   を置く。問いではないので `question` も `answer` も持たない。
--
--   **表も列も増えません。** 許す値が1つ増えるだけで、
--   すでに入っている行は1つも書き換わりません。何度貼っても同じです。
-- ══════════════════════════════════════════════════════════════════════
alter table public.material_sections drop constraint if exists material_sections_type_check;
alter table public.material_sections
  add constraint material_sections_type_check check (exercise_type in (
...
--
-- 【何が起きるか】
--   `qr_reviews` の表に、列が1つ増えます(`source`)。
--   **いまある行はすべて `sentence` になります**(この日まで溜めていたのは
--   文章だけだったので、推測ではありません)。
--   あわせて `mark_qr()` と `qr_items()` を作り直します。
--
-- 【どこまで影響するか】
--   教材・宿題・ゲストの情報・単語帳には触れません。
--
-- 【成功の目安】
--   `Success. No rows returned` と出れば成功です。
-- ============================================================================
...
--   **表も列も行も増えません。** いままで作れた種類も1つも減りません。
--
--   テストの中身は `translate_ja_en`(和文英訳)で、これは 0007 からある
--   演習の種類です。**そちらは触りません。**
--
--   **一覧は、いちばん新しいものにそろえてあります**(上の 0043・0047 の段と
--   同じ一覧です)。狭い段が1つでも混ざると、その値が入っている DB に
--   貼り直したとき "violated by some row" で止まります
--   (2026-09 に実機で踏みました・CLAUDE.md)。
