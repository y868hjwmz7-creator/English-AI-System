...
const SECTION_INSTRUCTIONS: Record<string, string> = {
  ...
  dialogue:
    ...
    + '登場人物の名前は、最初から最後まで変えない。'
    + '指定された数の発言で、1本の会話になること。'
...
const emitSectionTool = (sectionType: string, isFirst: boolean) => {
  ...
  if (isPassage) {
    ...
    /* **何の話だったか**(0046・2026-09 利用者の指定
         「同じ条件で作っても、過去の内容に被らないようにしたい」)。

       次に同じ業界・同じ場面で作るとき、**これを渡して避けさせる。**
       見出し(8語以内)だけでは筋まで分からず、
       「会議に遅れた新人が謝る話」と「打ち合わせに遅れた新人が謝る話」を
       見分けられない。**作るときに1回だけ書かせる**(数十トークン)。
       開くたびに要約させると、費用が毎回かかる
       (`headline_ja`・発音記号・カタマリの訳と同じ考え方)。

       **ゲストの画面には出さない。** 教材を選ぶための控えである。 */
    props.gist = {
      type: 'string',
      description: 'この本文が何の話かを、日本語1〜2文(60字以内)で。'
        + '**誰が・どこで・何をして・何が起きたか**を具体的に書く。'
        + '「〜について書いた記事」のような、中身の分からない言い方にしない',
    }
    required.push('gist')
  }
  if (isFirst) {
...
Deno.serve(async (req) => {
  ...
  /* **会話に出す人数**(2026-09 利用者の要望「会議というジャンルを作りたい」)。
     ...
     **上限は4人。** 14 発言を5人で分けると1人 2〜3 回しか話さず、
     その人らしさが出ないまま終わる(理由は `clipVoices.js` の
  ...
  /* **すでにある「話」**(0046・2026-09 利用者の指定)。
       > 選んだシチュエーションや場面が同じでも、
       > 全然違う感じになって欲しいわけです。

     `avoid`(上)は**英文**である。文が1つも一致しなくても、
     「会議に遅れた新人が上司に謝る話」と「打ち合わせに遅れた新人が
     先輩に謝る話」は、ゲストから見れば同じ話である。
     **見ている単位を、文から話へ上げる。**

     渡ってくるのは、同じ業界・同じ場面で作った過去の教材の
     見出しと筋(`materials.gist`)。**15本まで**(入力側なので
     1回あたり 300 トークンほど = 0.1円未満)。 */
  const avoidTopics = (Array.isArray(body.avoidTopics) ? body.avoidTopics : [])
    .map((t) => String(t ?? '').trim()).filter(Boolean).slice(0, 15)
  /* **話の切り口**(`src/data/materialAngles.js`)。
     同じ場面でも、切り口が違えばまったく別の話になる。

     **一覧は画面が持ち、ここへは組み立てた文が渡ってくる**
  ...
  const userPrompt = [
    ...
    subject
      ...
      : (isPassage ? '\n# 話題\n指定なし。業界とジャンルに合う、具体的で面白い話題を自分で決めること。'
        + '当たり障りのない一般論にしない。' : ''),
    /* **切り口**(0046)。場面が同じでも、角度が違えば別の話になる */
    angle && isPassage ? `\n# 切り口(この角度から書くこと)\n${angle}` : '',
    /* **具体を先に決めさせる**(0046・2026-09)。
       量産っぽく読める最大の原因は、AI が**当たり障りのない一般論**を
       書くことである。「近年、〇〇が注目されています」で始まる文章は、
       業界を変えても場面を変えても同じ形になる。
       **具体が入れば、それだけで別の話になる。**

       **実在の人物・企業の名前は使わせない**(`speechStyles.js` で
       決めた決まりと同じ。その人の言葉をなぞらせないため)。 */
    isPassage
      ? `\n# 書き始める前に、具体を決めること\n`
        + `- 登場する**人物か会社**(実在しない名前)・**場所**・**時期**・`
        + `**数字**を、書き始める前に決める\n`
        + `- 決めた具体を、本文の中に必ず出す\n`
        + `- **一般論で書き出さない。**「近年、〜が注目されています」`
        + `「〜は重要です」のような書き出しにしない\n`
        + `- **実在の人物・企業・商品の名前は使わない**`
      : '',
    context ? `\n# 本文(この内容から作ること)\n${context}` : '',
    ...
    /* **会話に出す人数**(2026-09 利用者の要望「会議というジャンル」)。
       2人なら1対1、3人以上なら会議・打ち合わせになる。
       人数だけを言っても「3人目がひとことだけ出て消える」ことがあるので、
       **1人あたり何回話すか**まで数えさせる。 */
    sectionType === 'dialogue'
      ? `\n# 登場人物(${speakers}人)\n`
        ...
        + (speakers === 2
          ...
          : `\n- **${speakers} 人が全員、${Math.floor(count / speakers)} 回以上話す。**`
            + `ひとことだけ出て消える人を作らない`
            + `\n- 順番に一巡させない。**話が進むように、相づち・割り込み・`
            + `同意・反論を混ぜる。** 実際の会議はそう進む`
            + `\n- 誰の意見なのかが分かるように書く(立場・役割を出す)`)
      : '',
    ``,
    countLine,
    ...
    avoid.length
      ...
      : '',
    /* **すでにある「話」**(0046)。上の `avoid` は文、こちらは筋である。
       **英文が1つも一致しなくても、筋が同じなら「また同じ話」になる。** */
    avoidTopics.length
      ? `\n# すでにある話(**この筋と似た話は書かないこと**)\n`
        + avoidTopics.map((t) => `- ${t}`).join('\n')
        + `\n\nこれらは、同じ業界・同じ場面ですでに作った教材である。`
        + `\n- **登場人物の立場・起きる出来事・結末が似ていたら、それは同じ話である。**`
        + `語や場所を入れ替えただけのものを作らない`
        + `\n- 場面の指定はそのまま守る。**変えるのは、その場面で何が起きるかである**`
      : '',
  ...
  const runOnce = async (retryNote: string) => {
    ...
    return {
      ...
      // 何の話だったか(0046)。**次に同じ場面で作るとき、これを渡して避けさせる**
      gist: result.gist ?? null,
