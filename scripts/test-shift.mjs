...
head('言い換えは、英文を英文に言い換える(第5.198節)')
{
  ...
  /* **言葉は、ボタンから箱の読み上げの名前へ移った**(第5.262節・
     2026-09-26 利用者の指定「『英語を見る』ボタンは廃止」)。
     **消したのは文字だけで、言葉は消していない**(黙って消さない)。

     書き分けは `revealLabel()` 1か所なので、
     **画面が `'answer'` と `'en'` を渡し分けているか**を見る ——
     言い換え(英文が問)では「答えを見る」、
     日本語 → 英語では「英語を見る」になる。 */
  ok(/revealLabel\(false, pair\.askEn \? 'answer' : 'en'\)/.test(card),
    '言い換えでは「答えを見る」、日本語 → 英語では「英語を見る」と名乗る')
  /* **ボタンは残っていないか。** 残っていたら、箱と同じことをするものが
     2つ並ぶ(同じことをするものを2つ見せない・CLAUDE.md) */
  ok(!/'答えを見る'|'英語を見る'/.test(card),
    '「答えを見る / 英語を見る」のボタンは、もう無い')
}
