...
/* **保存するファイルの名前**(第5.229節)。何にも依存しないので、
   素の node からそのまま呼べる */
import { hasDate, materialFileName, titleToName } from '../src/lib/fileName.js'
import {
...
/* ══ **保存するファイルの名前の決まり**(第5.229節・2026-09 利用者の指定)══

     > PDFや音声を保存する際に、デフォルトでファイル名の規則を
     > 指定しておきたい。日付_教材タイトル_レベル

   **ご提案のままにしなかった。** 教材の題名は、もう
   `YYYY-MM-DD / 主題 / 弱点 / レベル / 業種` の形で作られているので、
   「日付_題名_レベル」にすると**日付とレベルが2回ずつ**出る。
   利用者の選択は「**題名をそのまま使う**」。区切りを `_` に変えるだけ。

   **ここで数えるのは、その二重がいちばん大事である** ——
   「名前が付いているか」だけでは、二重になっても緑のままになる。
   ══════════════════════════════════════════════════════════════════ */
{
  const 題 = '2026-09-21 / 会議で反対意見を言う / 仮定法 / B1 / IT'
  /* **いちばん危ない形を置く**(CLAUDE.md)。
     題名に日付があり、**しかも作った日も分かっている**教材 ——
     ここで足してしまうと日付が2回出る。
     `created_at` を書かずに測ると、**足す道を通らないので素通りする**
     (実際、書き忘れていて赤チェックが緑のままだった) */
  const 教材 = { title: 題, created_at: '2026-09-15T08:00:00Z' }
  const 紙 = materialFileName(教材, 'quiz', 'pdf')
  const 音 = materialFileName(教材, 'audio', 'mp3')

  if (紙 !== '2026-09-21_会議で反対意見を言う_仮定法_B1_IT_問題.pdf') {
    ng('ファイル名 … 題名をそのまま使っていない', 紙)
  } else ok(`ファイル名 … ${紙}`)

  /* **二重になっていないか。** ここが今回の眼目である */
  if ((紙.match(/\d{4}-\d{2}-\d{2}/g) ?? []).length !== 1) {
    ng('ファイル名 … 日付が2回出ている', 紙)
  }
  else if ((紙.match(/B1/g) ?? []).length !== 1) ng('ファイル名 … レベルが2回出ている', 紙)
  else ok('ファイル名 … 日付もレベルも2回出ない')

  /* **PDF と音声は、題名の部分が同じ。** フォルダの中で隣同士に並ぶ */
  if (紙.replace(/_問題\.pdf$/, '') !== 音.replace(/_音声\.mp3$/, '')) {
    ng('ファイル名 … PDF と音声で題名の部分が違う', `${紙} / ${音}`)
  } else ok(`ファイル名 … PDF と音声が隣に並ぶ(${音})`)

  /* **日付が無い題名(手で付けたもの)にだけ、作った日を足す。**
     **無ければ足さない** —— 嘘の日付を作らない(CLAUDE.md) */
  const 手 = materialFileName(
    { title: '出張の打ち合わせ', created_at: '2026-09-18T02:11:00Z' }, 'full', 'pdf')
  const 日なし = materialFileName({ title: '出張の打ち合わせ' }, 'audio', 'mp3')
  if (手 !== '2026-09-18_出張の打ち合わせ_解答つき.pdf') {
    ng('ファイル名 … 日付が無い題名に、作った日を足していない', 手)
  } else if (日なし !== '出張の打ち合わせ_音声.mp3') {
    ng('ファイル名 … 作った日が分からないのに、日付を作っている', 日なし)
  } else ok(`ファイル名 … 日付が無ければ作った日(${手})・分からなければ足さない`)

  /* **いちばん危ない形を、必ず1つ置く**(CLAUDE.md)。
     空白なしの `/` は弱点の名前そのものに入っている(`分詞(ing/ed)`) */
  if (titleToName('2026-09-04 / 少子化 / 分詞(ing/ed)') !== '2026-09-04_少子化_分詞(ing ed)') {
    ng('ファイル名 … 空白なしの「/」で割ってしまい、弱点の名前が壊れる',
      titleToName('2026-09-04 / 少子化 / 分詞(ing/ed)'))
  } else ok('ファイル名 … 空白なしの「/」では割らない')

  const 記号 = materialFileName({ title: 'A/B:テスト*の話?' }, 'audio', 'mp3')
  if (/[\\/:*?"<>|]/.test(記号)) ng('ファイル名 … 保存できない字が残っている', 記号)
  else if (materialFileName({}, 'quiz', 'pdf') !== '教材_問題.pdf') {
    ng('ファイル名 … 題名が無いときの控えが無い', materialFileName({}, 'quiz', 'pdf'))
  } else if (materialFileName({ title: 'あ'.repeat(200) }, 'audio', 'mp3').length >= 100) {
    ng('ファイル名 … 長すぎる題名を切っていない')
  } else ok('ファイル名 … 記号を落とす・題名が無くても付く・長すぎれば切る')

  /* **先頭だけを日付と見る。** 途中にあるものを日付と読むと、
     足すべきところで足さなくなる */
  if (!hasDate('2026-09-21 / x') || hasDate('x / 2026-09-21')) {
    ng('ファイル名 … 日付を、先頭だけで見ていない')
  } else ok('ファイル名 … 日付は先頭だけを見る')
}
 ...
 * ══════════════════════════════════════════════════════════════════ */
{
  ...
  // ⓓ **道が1本も切れていないか**(窓口 → `.json` → 画面 → 鳴らす側)
  {
    ...
    /* **区切りは信じる。ただし継ぎ目に配ったぶんは見積もりである**
       (第5.218節)。配ったときまで「確か」にすると、守りの余裕が外れる */
    if (!/\(fit\.how === 'segments' && !\(fit\.per > 0\)\)/.test(read)) {
      miss.push('向こうの区切りを、信じる側に入れていない')
    }
    /* **区切りのあとに、時計も合わせていること**(第5.218節)。
       `voice_segments` は控えと同じ時計なので、これが無いと
       **余った秒がまるごと残る**(5回ぶんの不具合そのもの) */
    if (!/if \(fit\.per > 0\) \{\n\s+spans = shiftSeams\(spans, fit\.per\)/.test(read)) {
      miss.push('区切りのあとに、時計を合わせていない')
    }
    if (!/per: base\.how === 'seam' \? base\.per : 0/.test(read)) {
      miss.push('継ぎ目に配るぶんを、区切りの側へ渡していない')
    }
...
/* ══════════════════════════════════════════════════════════════════
   ⑰ **本物の数字**(第5.218節・2026-09 利用者が送ってくれたもの)

   5回直しても直らなかったのは、**本物のデータを1度も見ていなかった**
   からである。利用者に「この数字をコピー」を押してもらって、初めて出た。
   ...
   **`voice_segments` は無音を含んでいない。** 32手めは
   「向こうが本当の秒を返している」と読んだが、返していたのは
   **時刻表と同じ時計での区切り**だった。ところがこちらは
   「区切りが返っている＝合っている」とみなし、**残りの 1.41 秒を
   配る処理を丸ごと飛ばしていた。**

   **区切りと、時計合わせは、別の仕事である。**
   ここは**その本物の数字をそのまま**持っている。作り物ではない。
   ══════════════════════════════════════════════════════════════════ */
console.log('\n── ⑰ 本物の数字(第5.218節)──')
{
  const { clockFitOf: fitOf2, fitWindows: win2, segWindowsOf: segWin2, shiftSeams: seam2,
    indexAtTime: at2 } = await import('../src/lib/wholeAudio.js')

  /** 利用者の端末から届いた `voice_segments`(始まり, 終わり, 文字の頭, 文字の尻) */
  const RAW = [
    [0, 2.399, 0, 43], [2.399, 7.44, 43, 115], [7.44, 8.56, 115, 136],
    [8.56, 12.56, 136, 190], [12.56, 22.16, 190, 340], [22.16, 25.88, 340, 396],
    [25.88, 39.68, 396, 615], [39.68, 42.401, 615, 647], [42.401, 48.399, 647, 749],
    [48.399, 55.519, 749, 846], [55.519, 64.0, 846, 964],
  ]
  /** 同じときの、こちらが出していた文の区間(発言, 頭, 尻) */
  const SENT = [
    [0, 0, 1.52], [0, 1.56, 2.399], [1, 2.399, 4.56], [1, 4.64, 6.8], [2, 6.92, 8.04],
    [2, 8.064, 8.56], [3, 8.56, 11.2], [3, 11.4, 12.56], [4, 12.56, 16.96],
    [4, 17.04, 20.08], [4, 20.14, 22.16], [5, 22.16, 23.52], [5, 23.68, 25.88],
    [6, 25.88, 30.56], [6, 30.7, 34.48], [6, 34.66, 37.44], [6, 37.573, 39.04],
    [7, 39.11, 40.72], [7, 40.8, 42.401], [8, 42.401, 45.68], [8, 45.787, 48.399],
    [9, 48.399, 52.96], [9, 53.093, 55.519], [10, 55.519, 56.4], [10, 56.6, 59.76],
    [10, 59.96, 64.0],
  ]
  const ALIGN_END = 64.0
  const DUR = 65.41

  const segs = RAW.map(([a2, b2, cs, ce], i) => ({
    dialogue_input_index: i, start_time_seconds: a2, end_time_seconds: b2,
    character_start_index: cs, character_end_index: ce,
  }))
  const items = RAW.map((_, i) => {
    const m = SENT.filter((x) => x[0] === i)
    return { start: m[0][1], end: m[m.length - 1][2] }
  })
  const sent = SENT.map(([item, start, end]) => ({ item, start, end }))

  /* ① **継ぎ目に配る、と決まること。** ここが `same` に倒れると、
        1.41 秒がまるごと残る(それが5回ぶんの不具合だった) */
  const base2 = fitOf2(items, ALIGN_END, DUR)
  if (base2.how !== 'seam') {
    ng('本物の数字 … 継ぎ目に配る形になっていない', `${base2.how}`)
  } else if (Math.abs(base2.per - 1.41 / 10) > 0.001) {
    ng('本物の数字 … 1か所あたりの配りが違う', `${base2.per}`)
  } else ok(`本物の数字 … 継ぎ目に配る(1か所 ${base2.per.toFixed(3)} 秒)`)

  /* ② **区切りだけでは何も直らないこと。**
        `voice_segments` は控えと同じ時計なので、最後は 64.00 のまま */
  const wins2 = segWin2(segs, items)
  if (!wins2) ng('本物の数字 … 発言の窓が取れていない')
  else {
    const 窓だけ = win2(sent, wins2)
    const 窓の尻 = 窓だけ[窓だけ.length - 1].end
    if (Math.abs(窓の尻 - ALIGN_END) > 0.01) {
      ng('本物の数字 … 区切りだけで音声の長さに届いている(この教材では届かないはず)',
        `${窓の尻}`)
    } else ok(`本物の数字 … 区切りだけでは ${窓の尻.toFixed(2)} 秒のまま(音声は ${DUR} 秒)`)

    /* ③ **両方かけて、はじめて音声の終わりに届く。**
          これが直ったことの証拠である */
    const 直した = seam2(窓だけ, base2.per)
    const 尻 = 直した[直した.length - 1].end
    if (Math.abs(尻 - DUR) > 0.01) {
      ng('本物の数字 … 直しても音声の終わりに届かない', `${尻.toFixed(2)} / ${DUR}`)
    } else ok(`本物の数字 … 区切り + 時計で、最後の文が ${尻.toFixed(2)} 秒(音声とぴたり)`)

    /* ④ **前へ戻らないこと。** 2つ重ねるので、並びが乱れたら光りが戻る */
    const 乱れ = 直した.filter((x, i) => i > 0 && x.start < 直した[i - 1].start - 1e-9)
    if (乱れ.length) ng('本物の数字 … 文の並びが乱れている', `${乱れ.length} 件`)
    else ok('本物の数字 … 文の並びは乱れない(光りが戻らない)')

    /* ⑤ **後ろの発言ほど、大きくずらすこと。** 均等に配っているのだから、
          発言 0 は動かず、最後は 1.41 秒動く */
    const 頭 = (i) => 直した.find((x) => x.item === i).start - sent.find((x) => x.item === i).start
    if (Math.abs(頭(0)) > 0.01) ng('本物の数字 … 1つめの発言まで動かしている', `${頭(0)}`)
    else if (!(頭(10) > 頭(5) && 頭(5) > 頭(1))) {
      ng('本物の数字 … 後ろの発言ほど大きくずれていない',
        `${頭(1).toFixed(2)} / ${頭(5).toFixed(2)} / ${頭(10).toFixed(2)}`)
    } else ok(`本物の数字 … 発言 0 は動かさず、最後は ${頭(10).toFixed(2)} 秒ずらす`)

    /* ⑥ **直す前は、本当に音声より先に進んでいたこと。**
          これが無いと「直っていないものを直した」ことの確認にならない */
    /* **控えの中にある秒で見る。** 終わりを越えた秒だと、
       どちらも「いちばん最後」を返して差が出ない(見張りにならない) */
    const 先に進む = at2(sent, 56.5) !== at2(直した, 56.5)
    if (!先に進む) ng('本物の数字 … 直す前と後で、光る文が変わらない(直っていない)')
    else ok('本物の数字 … 直す前は、終わりのほうで別の文を光らせていた')
  }
}
