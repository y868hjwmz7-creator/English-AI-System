   ...
   ──────────────────────────────────────────────────────────── */
{
  ...
  /* ── **窓口が古いときの知らせ**(第5.249節・2026-09-23 実機)───────────
       利用者の指摘「**なぜ作れない**」。画面にはこう出ていた。

         作れませんでした。演習の種類が正しくありません: vocab_recall

       演習を1つ足した日に、**古い窓口はその名前を知らない。**
       断りの文をそのまま出すと、利用者には
       **こちらの作りかけなのか、配布が落ちたのか**が分からない。 */
  {
    /* **切り出しの終わりを `\n}` にしない。** 引数の分割代入の閉じ括弧が
       すでに行頭の `}` なので、**中身が1文字も入らない**(2026-09-23 に踏んだ)。
       関数の終わりは `\n}` の**次が改行**のところである */
    const gen = /export async function generateSection\(\{[\s\S]*?\n}\n/.exec(matS)?.[0] ?? ''
    ok(Boolean(gen) && /演習の種類が正しくありません/.test(gen)
       && /staleGen\(/.test(gen),
      '窓口が古い … 教材を作る道でも、断りを「窓口が古い」と読み替える')
    /* **「出る」と「出ない」の両方を見る**(CLAUDE.md)。
       何でもかんでも「窓口が古い」にすると、**本当の理由が消える** */
    ok(Boolean(gen) && /return ng\(detail \|\|/.test(gen),
      '窓口が古い … ほかの断りは、理由をそのまま出す')
    /* **頼むことは1か所**(呼び名を2か所に書かない)。
       数えるのは `staleGen(` を**使っている**数 —— 4 か所ある */
    const 使用 = (matS.match(/staleGen\('/g) ?? []).length
    ok(使用 >= 4, '窓口が古い … 頼む文は1か所で作る', `${使用} か所`)
    /* **窓口はもう手で貼らない**(第5.233節)。GitHub が配る。
       「置き直してください」と書くと、**できない作業を頼むことになる** */
    ok(!/Edge Functions → generate-material を置き直/.test(matS),
      '窓口が古い … 手で置き直せとは書かない(GitHub が配る)')
  }
   ...
   ══════════════════════════════════════════════════════════════════════ */
{
     ...
     ══════════════════════════════════════════════════════════════ */
  {
    ...
    ok(!/fill="currentColor"/.test(speaker),
      '🔊 は塗りつぶさない(となりの絞り込みと同じ線画)')
    /* **太さを書き写さない。** 絞り込みの側から読んで突き合わせる ——
       どちらかを変えた日に、**片方だけ**が変わっていれば赤くなる。
       **第5.275節で絵は変わったが、太さは 1.6 のまま**である ——
       利用者が「太すぎる」と言ったのは**形**のほうだった */
   ...
   ──────────────────────────────────────────────────────────────── */
{
  ...
  /* ══ **絞り込みの絵**(第5.275節・2026-09-26 実機・利用者の指定)══════

       > 絞り込みのマークがダサいですね。線が太すぎて横の聞き流しの
       > スピーカーマークとの統一感がないです。
       > 絞り込みのマークは添付のものにしてください

     ここまで2度変わっている。**戻らないように、性質で見張る。**

       第5.184節 … 長さの違う横3本線 → 運動靴の印に見えると言われた
       第5.262節 … じょうご(ろうと) → 下がすぼまって黒い棒に見えた
       第5.275節 … **つまみ3本**(いま)

     **値を書き写さず、性質で見る**(CLAUDE.md)。
     つまみ3本の性質は4つ ——
       ①線が3段 ②つまみが3つ ③つまみは輪(塗りつぶさない)
       ④**つまみの位置が3つとも違う**(そろえると「三本線」に戻る)
     じょうごに戻せば①②が、ただの3本線に戻せば②④が赤くなる。 */
  ok(/export function SortIcon\b/.test(icons), 'ソートアイコンがある(`SortIcon`)')
  {
    const svg = icons.match(/export function SortIcon[\s\S]*?\n\}/)?.[0] ?? ''
    /** つまみ(輪)を読む */
    const 輪 = [...svg.matchAll(
      /<circle\s+cx="([\d.]+)"\s+cy="([\d.]+)"\s+r="([\d.]+)"/g)]
      .map((m) => ({ x: Number(m[1]), y: Number(m[2]), r: Number(m[3]) }))
    /** 線(横棒)を読む。`M x y h len` の形 */
    const 線 = [...svg.matchAll(/d="M([\d.]+)\s+([\d.]+)h([\d.]+)"/g)]
    ...
    ok(!/<circle[^>]*fill="currentColor"/.test(svg) && /fill="none"/.test(svg),
      'つまみは輪(塗りつぶしていない)')
    /* ★ **線が、輪の中を横切っていないか。**
         ここを見ないと、**つまみの穴を線が突っ切った絵**でも緑になる。
         輪の外側(半径 + 線の太さの半分)と、線の端(丸い始末のぶん)が
         重なっていないことを、段ごとに確かめる */
    let 突っ切り = 0
    for (const c of 輪) {
      const 外 = c.r + 太さ / 2
      for (const l of 線.filter((x) => Math.abs(x.y - c.y) < 0.01)) {
        const 左 = l.x - 太さ / 2
        const 右 = l.x + l.len + 太さ / 2
        if (右 > c.x - 外 && 左 < c.x + 外) 突っ切り += 1
      }
    }
    ok(突っ切り === 0,
      '線は、つまみの手前で止まっている(輪の中を横切らない)',
      `${突っ切り} 本が重なっている`)
    /* ★ **つまみどうしが、くっついていないか**(共通ルール
         「別々の物を、すき間ゼロでくっつけない」)。

         **実際に描いて見つけた。** 輪を大きく(半径 2.1)したら、
         2段目と3段目の輪が重なって1つの塊に見えた。
         輪の大きさと段の間は**いっしょに決まる** ——
         小さすぎれば穴がふさがって点に見え、大きすぎれば隣とくっつく。 */
    let くっつき = 0
    for (let i = 0; i < 輪.length; i += 1) {
      for (let j = i + 1; j < 輪.length; j += 1) {
        const a = 輪[i]
        const b = 輪[j]
        const 間 = Math.hypot(a.x - b.x, a.y - b.y)
        // 外まわり(半径 + 線の太さの半分)どうしが触れていないこと
        if (間 < (a.r + 太さ / 2) + (b.r + 太さ / 2)) くっつき += 1
      }
    }
    ok(くっつき === 0, 'つまみどうしが、くっついていない', `${くっつき} 組が重なっている`)
    /* **段ごとに、左右2本ずつある**(片側だけだと目盛りに見えない) */
    const 段ごと = {}
    for (const l of 線) 段ごと[l.y] = (段ごと[l.y] ?? 0) + 1
    ok(Object.values(段ごと).every((n) => n === 2),
      'どの段も、つまみの左右に線がある',
      Object.entries(段ごと).map(([y, n]) => `${y}:${n}`).join(' / '))
  }
...
/* ────────────────────────────────────────────────────────────────
   第5.223節 フックの決まりを、誰も外していない

   2026-09。`LearnerHomework` が真っ白になった原因(第5.220節)は、
   **フックを早い `return` の後ろに足した**ことだった。
   `npm run lint` に `react-hooks/rules-of-hooks` を入れて塞いだが、
   **そのとき `LessonView.jsx` だけ名指しで外してあった**
   (同じ形が 10 か所あり、言われていない画面だったため)。

   利用者の指示で 10 か所を直し、**名指しの除外も消した**(第5.223節)。

   **ここで見るのは「誰も外していないか」である。**
...
console.log('\n── フックの決まり(第5.223節)──')
{
  ...
  ok(/'react-hooks\/rules-of-hooks': 'error'/.test(es),
    'フックの決まり … `rules-of-hooks` が `error` で入っている')
  /* **「出る」と「出ない」の両方を見る。** 入っているだけでは足りない ——
     どこかで `'off'` にしていれば、そのファイルは素通りする */
  ok(!/'react-hooks\/rules-of-hooks': '(off|warn)'/.test(es),
    'フックの決まり … 名指しで外しているファイルが1つも無い')
}
...
/* ==========================================================================
 * **発行したあとに残る帯を、片づける**(第5.265節・2026-09-26 実機)
 *
 *   > 教材が完成して発行した後もこの帯がしつこく残ります。
 *   > これを改善してください。
 *
 *   残っていたのは**2つ**で、出どころが別だった。
 *
 *   | 何 | いつ消えていたか |
 *   |---|---|
 *   | 「◯◯の下書きができました」(緑の知らせ) | **×を押すまで** |
 *   | 「… の支度ができました(…)」(上の帯) | **「閉じる」を押すまで** |
 * ========================================================================== */
{
  console.log('\n▶ 発行したあとの知らせ')
  const noC5 = (t) => t.replace(/\/\*[\s\S]*?\*\/|\{\/\*[\s\S]*?\*\/\}/g, '')
  const readS5 = (f) => readFileSync(new URL(`../${f}`, import.meta.url), 'utf8')
  const prep = noC5(readS5('src/lib/prepareJob.js'))
  const app = noC5(readS5('src/App.jsx'))

  /* ── ① **うまくいった知らせは、ひとりでに閉じる** ── */
  ok(/export const PREP_HIDE_MS = \d+/.test(prep) && /hideLater\(mine\)/.test(prep),
    '支度の知らせ … うまくいったら、ひとりでに閉じる')
  /* ── ② **失敗は消さない**(成功と失敗を、同じ見た目で終わらせない)── */
  ok(/if \(task\.error\) return/.test(prep),
    '支度の知らせ … 失敗したときは消さない(理由が残る)')
  /* **失敗の道からは、閉じにいかない。** ①だけだと、
     `catch` の側にも足してしまっても緑のままになる */
  ok(!/error: e\?\.message[\s\S]{0,200}hideLater/.test(prep),
    '支度の知らせ … 失敗の道からは、閉じにいかない')
  /* ── ③ **次の支度の知らせを、消してしまわない** ── */
  ok(/if \(task\?\.id !== id\) return/.test(prep),
    '支度の知らせ … 次の支度が始まっていたら、そちらは消さない')
  /* **まだ走っているものも消さない**(「閉じる」でも同じ決まり) */
  ok(/if \(task\.state !== 'done'\) return/.test(prep),
    '支度の知らせ … まだ走っているあいだは消さない')

  /* ── ④ **下書きの知らせは、仕事が片づいたら一緒に消える** ──
       受け取った時点(`takeJobResult()`)で帯は消えるのに、
       **知らせだけが別の入れ物**なので残っていた。
       しかも押す先は、もう受け取り済みの画面である
       (**効かない操作を見せない**・CLAUDE.md)。 */
  ok(/if \(!j\) \{ setJobNote\(null\); return \}/.test(app),
    '下書きの知らせ … 受け取ったら(発行へ進んだら)一緒に消える')
...
/* ══════════════════════════════════════════════════════════════════
   **意味は、語句そのものの意味に限る**(第5.272節・2026-09-26 実機)

     > 単語帳の出題で一列になって進むと翻訳されるが
     > 元ネタの文章の文脈と意味が違う
     > Expected it to matter → 「予想していなかった」と翻訳され、
     > 意味が否定形の選択肢で出題される(元ネタの主語が Nobody のため)

   単語帳に出る意味は `word_glosses` の `meaning_ja` で、
   **書いているのは `lookup-word` だけ**である(`mark_word` は意味を持たない)。
   指示は「その文でふさわしい意味を先頭に」とだけ言っていて、
   **「語句そのものの意味に限る」と言っていなかった。**
   だから文の否定(Nobody)が意味のほうへ流れ込んだ。

   **見るのは、指示文の中身である。**
   コメントに書いてあっても AI には届かないので、
   **`SYSTEM_PROMPT` の文字列の中**を見る(「名前が出てくるか」で見ない)。
   ══════════════════════════════════════════════════════════════════ */
{
  const readG = (f) => readFileSync(new URL(`../${f}`, import.meta.url), 'utf8')
  const src = readG('supabase/functions/lookup-word/index.ts')
  /* **指示文そのものを切り出す。** ファイル全体を探すと、
     この上のコメントに同じ語を書いた日に**素通りする** */
  const m = src.match(/const SYSTEM_PROMPT = `([\s\S]*?)`\n/)
  if (!m) {
    ok(false, '語の意味 … `SYSTEM_PROMPT` を切り出せない(検証の前提が崩れている)')
  } else {
    const p2 = m[1]
    ok(/意味は渡された語句そのものの意味に限る/.test(p2),
      '語の意味 … 「語句そのものの意味に限る」と指示している')
    /* **文にあって語句に無いもの**を、名指しで禁じているか。
       「文脈を見て」だけでは、いまと同じことが起きる */
    ok(/否定・主語・時・疑問/.test(p2),
      '語の意味 … 持ち込ませないもの(否定・主語・時・疑問)を名指ししている')
    /* **起きた形そのものを、例として置いてあるか**(利用者の報告)。
       決まりだけだと、AI は「文脈に合わせた」と思って同じことをする */
    ok(/Nobody expected it to matter/.test(p2) && /予想していなかった.*誤り/s.test(p2),
      '語の意味 … 実際に起きた形(Nobody 〜)を、誤りの例として置いている')
    /* **なぜ効くのかを、指示文の中にも書いてあるか。**
       「単語帳にそのまま出る」を書かないと、AI は
       読解の助けとしてふさわしい訳を選びに行く */
    ok(/単語帳の出題にそのまま出る/.test(p2),
      '語の意味 … この意味が単語帳の出題に出ることを、指示文で伝えている')
  }
  /* **「その文でふさわしい意味を先頭に」を落としていないか**
     (出る / 出ないの両方・CLAUDE.md)。
     語句だけを見るようにすると、bank が「土手」になって戻る */
  ok(/その文でふさわしい意味を必ず先頭に置く/.test(src),
